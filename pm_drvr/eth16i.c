...
static char *version = "eth16i.c: v0.21 17-10-95 Mika Kuoppala (miku@elt.icl.fi)\n";
 ...
 */
static DWORD eth16i_portlist[] = {
             0x260, 0x280, 0x2A0, 0x240, 0x340, 0x320, 0x380, 0x300, 0
           };

static DWORD eth32i_portlist[] = {
 ...
 */
static DWORD eth16i_irqmap[] = {
             9, 10, 5, 15
           };

/* This is the Interrupt lookup table for Eth32i card
 */
static DWORD eth32i_irqmap[] = {
             3, 5, 7, 9, 10, 11, 12, 15
           };

#define EISA_IRQ_REG  0xc89

static DWORD eth16i_tx_buf_map[] = {
             2048, 2048, 4096, 8192
           };
...
struct eth16i_local {
       ...
       DWORD  tx_started:1;
       BYTE   tx_queue;      /* Number of packets in transmit buffer */
       WORD   tx_queue_len;
       DWORD  tx_buf_size;
       DWORD  open_time;
     };
...
static void  eth16i_send_probe_packet (short ioaddr, BYTE * b, int l);
...
static void  eth16i_eeprom_cmd (int ioaddr, BYTE command);
static int   eth16i_open (struct device *dev);
static void  eth16i_close (struct device *dev);
static int   eth16i_tx (struct device *dev, const void *buf, int len);
static void  eth16i_rx (struct device *dev);
static void  eth16i_interrupt (int irq);
static void  eth16i_multicast (struct device *dev);
static void  eth16i_select_regbank (BYTE regbank, short ioaddr);
static void  eth16i_initialize (struct device *dev);
static void *eth16i_get_stats (struct device *dev);
...
static int eth16i_probe1 (struct device *dev, short ioaddr)
{
   ...
   */
  if (!eth16i_check_signature (ioaddr))	/* Can we find the signature here */
     return (0);
   ...
   */
  if (request_irq (dev->irq, &eth16i_interrupt))
  {
    printk ("%s: %s at base %3x, but is unusable due to conflicting IRQ %d.\n",
            dev->name, cardname, ioaddr, irq);
    return (0);
  }

  printk ("%s: %s at %#3x, IRQ %d, ", dev->name, cardname, ioaddr, dev->irq);
   ...
   */
  if (dev->priv == NULL)
     dev->priv = k_calloc (sizeof (struct eth16i_local), 1);

  dev->open      = eth16i_open;
  dev->close     = eth16i_close;
  dev->xmit      = eth16i_tx;
...
static void eth16i_initialize (struct device *dev)
{
  ...
  for (i = 0; i < 3; i++)
  {
    WORD node_val = eth16i_read_eeprom (ioaddr, E_NODEID_0 + i);
    ((WORD*)dev->dev_addr)[i] = ntohs (node_val);
  }

  for (i = 0; i < 6; i++)
  {
    outb (((BYTE*)dev->dev_addr)[i], ioaddr + NODE_ID_0 + i);
    if (boot)
    {
  ...
  if (boot)			/* Now set port type */
  {
    ...
    WORD ptype = eth16i_read_eeprom (ioaddr, E_PORT_SELECT);
...
static int eth16i_probe_port (short ioaddr)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    eth16i_send_probe_packet (ioaddr, dummy_packet, 64);
    if (eth16i_receive_probe_packet (ioaddr))
    {
      if (eth16i_debug > 1)
...
static void eth16i_send_probe_packet (short ioaddr, BYTE * b, int l)
{
  ...
  outsw (ioaddr + DATAPORT, (WORD *) b, (l + 1) >> 1);
...
static int eth16i_get_irq (short ioaddr)
{
  ...
  /* Oh..the card is EISA so method getting IRQ is different
   */
...
static int eth16i_check_signature (short ioaddr)
{
   ...
   * from which it couldn't be found anymore. If there is need
...
static int eth16i_tx (struct device *dev, const void *buf, int len)
{
  ...
  if (dev->tx_busy)
  {
    ...
    {
      printk
	("%s: timeout regs: %02x %02x %02x %02x %02x %02x %02x %02x.\n",
         dev->name,
         inb (ioaddr+0), inb (ioaddr+1), inb (ioaddr+2), inb (ioaddr+3),
         inb (ioaddr+4), inb (ioaddr+5), inb (ioaddr+6), inb (ioaddr+7));
  ...
  else
  {
    ...
    else if (lp->tx_queue_len < lp->tx_buf_size - (ETH_MAX + 2))
    {
...
static void eth16i_rx (struct device *dev)
{
  ...
  while ((inb (ioaddr + RECEIVE_MODE_REG) & RX_BUFFER_EMPTY) == 0)
  {
	...
	break;
       ...
       */
      if (!dev->get_rx_buf ||
          (pkt_buf = (*dev->get_rx_buf)(pkt_len)) == NULL)
      {
        lp->stats.rx_dropped++;
        break;
      }
      ...
      if (ioaddr < 0x1000)
         rep_insw (ioaddr + DATAPORT, (WORD*)pkt_buf, (pkt_len+1) >> 1);
      else
      {
	BYTE frag = pkt_len % 4;

        rep_insl (ioaddr + DATAPORT, (DWORD*)pkt_buf, pkt_len >> 2);
        ...
        if (frag)
	{
	  ...
	  if (frag == 3)
          ...
          memcpy (pkt_buf + (pkt_len & 0xfffc), (char*)rest, frag);
	}
      }
...
static void eth16i_interrupt (int irq)
{
  ...
  if (status & 0x00ff)
  {				/* Let's check the transmit status reg */
    if (status & TX_DONE)
    {				/* The transmit has been done */
      lp->stats.tx_packets++;
#if 0                           /* make this blocking Tx for simplicity */
      if (lp->tx_queue)
...
static void eth16i_select_regbank (BYTE banknbr, short ioaddr)
{
  BYTE data = inb (ioaddr + CONFIG_REG_1);
