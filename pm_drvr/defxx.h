...
/* Define basic types for BYTEs, shorts, longs */

typedef BYTE  PI_UINT8;
typedef WORD  PI_UINT16;
typedef DWORD PI_UINT32;
...
/* format of Type 1 Producer register
 */
typedef union {
        PI_UINT32 lword;
        struct {
...
/* format of Type 2 Producer register
 */
typedef union {
        PI_UINT32 lword;
        struct {
...
/* format of Type 1 Consumer Block longword
 */
typedef union {
        PI_UINT32 lword;
        struct {
...
/* format of Type 2 Consumer Block longword
 */
typedef union {
        PI_UINT32 lword;
        struct {
...
/* Define offsets into FDDI LLC transmit frame buffers -
 * used when sending frames
 */
...
typedef struct DFX_board_tag {
  ...
  DWORD              descr_block_phys; /* PDQ descriptor block phys address */
  PI_DMA_CMD_REQ    *cmd_req_virt;     /* Command request buffer virt address */
  DWORD              cmd_req_phys;     /* Command request buffer phys address */
  PI_DMA_CMD_RSP    *cmd_rsp_virt;     /* Command response buffer virt address */
  DWORD              cmd_rsp_phys;     /* Command response buffer phys address */
  char              *rcv_block_virt;   /* LLC host receive queue buf blk virt */
  DWORD              rcv_block_phys;   /* LLC host receive queue buf blk phys */
  PI_CONSUMER_BLOCK *cons_block_virt;  /* PDQ consumer block virt address */
  DWORD              cons_block_phys;  /* PDQ consumer block phys address */
   ...
   */
  BYTE  uc_table [FDDI_K_ALEN];
  DWORD uc_count;                      /* number of unicast addresses */
  BYTE  mc_table[PI_CMD_ADDR_FILTER_K_SIZE * FDDI_K_ALEN];
  DWORD mc_count;                      /* number of multicast addresses */

  /* Current packet filter settings */

  DWORD ind_group_prom;		/* LLC individual & group frame prom mode */
  DWORD group_prom;		/* LLC group (multicast) frame prom mode */

  /* Link available flag needed to determine whether to drop outgoing packet requests
   */
  DWORD link_available;		/* is link available? */

  /* Resources to indicate reset type when resetting adapter
   */
  DWORD reset_type;		/* skip or rerun diagnostics */
  ...
  DWORD  bus_type;               /* bus type (0 == PCI, 1 == EISA) */
  WORD   base_addr;              /* base I/O address (same as dev->base_addr) */
  BYTE   pci_bus;                /* PCI bus number */
  BYTE   pci_dev_fun;            /* PCI device and function numbers */
  DWORD  full_duplex_enb;        /* FDDI Full Duplex enable (1 == on, 2 == off) */
  DWORD  req_ttrt;               /* requested TTRT value (in 80ns units) */
  DWORD  burst_size;             /* adapter burst size (enumerated) */
  DWORD  rcv_bufs_to_post;       /* receive buffers to post for LLC host queue */
  BYTE   factory_mac_addr[FDDI_K_ALEN];   /* factory (on-board) MAC address */
  ...
  DWORD  rcv_discards;
  DWORD  rcv_crc_errors;
  DWORD  rcv_frame_status_errors;
  DWORD  rcv_length_errors;
  DWORD  rcv_total_frames;
  DWORD  rcv_multicast_frames;
  DWORD  xmt_discards;
  DWORD  xmt_length_errors;
  DWORD  xmt_total_frames;
} DFX_board_t;
