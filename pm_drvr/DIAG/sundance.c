...
"alta-diag.c:v0.05 8/18/99 Donald Becker (becker@cesdis.gsfc.nasa.gov)\n";
...
extern int hamachi_diag(int vend_id, int dev_id, long ioaddr, int part_idx);
extern int  vortex_diag(int vend_id, int dev_id, long ioaddr, int part_idx);
extern int    alta_diag(int vend_id, int dev_id, long ioaddr, int part_idx);
...
static u16 new_ee_contents[EEPROM_SIZE];
...
int sundance_diag(int vendor_id, int device_id, long ioaddr, int part_idx)
{
	...
	int mac_mode = inw(ioaddr + MACCtrl);
	int i, eesize = 8;

	if (mac_mode == 0xffff)
		printf("\nThe appears not to exist!  The following info is probably"
			   " bogus.\n\n");
	...
	if (set_hwaddr) {
...
#if defined(EEPROM_CSUM_OFFSET)  &&  EEPROM_CSUM_OFFSET > 0
		{
			...
			write_eeprom(ioaddr, EEPROM_CSUM_OFFSET, sum);
		}
#endif
		for (i = 0; i < eesize; i++)
			eeprom_contents[i] = read_eeprom(ioaddr, i);
	...
	if (show_eeprom) {
		...
		printf("  Configuration %4.4x, ASIC Control %4.4x.\n",
			    eeprom_contents[0], eeprom_contents[1]);
...
static int read_eeprom(long ioaddr, int location)
{
	...
	do {
		if (! (inw(ioaddr + EECtrl) & 0x8000)) {
			if (debug)
				printf("EEPROM read took %d ticks.\n", 1000 - boguscnt);
...
static void write_eeprom(long ioaddr, int location, int value)
{
	...
	outw(0x0100 | (location & 0xff), ioaddr + 0x36);
	do {
		if (! (inw(ioaddr + 0x36) & 0x8000))
			return;
	} while (--boguscnt > 0);
	fprintf(stderr, "EEPROM write of location %d with %#x failed!\n",
			location, value);
	return;
}
...
#define mdio_delay()  inb(mdio_addr) 	/* Extra bus turn-around as a delay. */
...
{
	int mdio_addr = ioaddr + MDIO_IO_OFFSET;
	int i;

	for (i = 32; i >= 0; i--) {
		outl(MDIO_WRITE1, mdio_addr);
		mdio_delay();
		outl(MDIO_WRITE1 | MDIO_SHIFT_CLK, mdio_addr);
		mdio_delay();
	}
	return;
}
...
/*
 * Local variables:
 *  compile-command: "cc -O -Wall -Wstrict-prototypes -o alta-diag alta-diag.c -DLIBFLASH libflash.c"
