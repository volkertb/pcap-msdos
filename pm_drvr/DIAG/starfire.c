/* starfire-diag.c: Diagnostic and setup core code for Linux ethercards.
...
"starfire-diag.c:v1.00 8/28/99 Donald Becker (becker@cesdis.gsfc.nasa.gov)\n";
...
extern int starfire_diag(int vend_id, int dev_id, int ioaddr, int part_idx);
...
int starfire_diag(int vendor_id, int device_id, int ioaddr, int part_idx)
{
	int i;
	...
	if (verbose || show_regs) {
		...
		printf("Tx producer index %d, consumer index %d.\n",
			   inw(ioaddr + 0xa0), inw(ioaddr + 0xa4));
		printf("Rx consumer index %d, producer index %d.\n",
			   inw(ioaddr + 0xea), inw(ioaddr + 0xe8));
		printf("PCI Bus maximum latency was %d nsec, peak interrupt latnecy "
			   "%d usec.\n",
			   (inl(ioaddr + 0x48)>>24)*480, inl(ioaddr + 0x48)*30);
