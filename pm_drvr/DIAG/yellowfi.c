...
"yellowfin-diag.c:v1.00 8/31/99 Donald Becker (becker@cesdis.gsfc.nasa.gov)\n";
static char *usage_msg =
"Usage: yellowfin-diag [-aEefFmqrRtvVwW] [-p <IOport>] [-[AF] <media>]\n";
...
int symbios_diag(int vendor_id, int device_id, long ioaddr, int part_idx)
{
	...
	if (show_eeprom > 1) {
		printf("EEPROM byte zero is %2.2x/%2.2x.\n",
			   read_eeprom(ioaddr, 0), read_eeprom_serial(ioaddr, 0));
