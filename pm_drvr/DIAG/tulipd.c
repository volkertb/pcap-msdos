...
struct pcidev_entry {
  ...
  { "ADMtek AL981 Comet", "ADMtek AL981 Comet",
	0x1317, 0x0981, 0xffff, COMET_MII | COMET_MACADDR, 256, tulip_diag },
  { "Davicom DM9102", "Davicom DM9102",
	0x1282, 0x9102, 0xffff, CSR12_IS_GPIO, 128, tulip_diag },
  { "Davicom DM9100", "Davicom DM9100",
	0x1282, 0x9100, 0xffff, CSR12_IS_GPIO, 128, tulip_diag },
  { "Davicom DM9100 series", "Davicom DM9100 series (unknown model)",
	0x1282, 0x9100, 0xfff0, CSR12_IS_GPIO, 128, tulip_diag },
	{ 0, 0, 0, 0},
};
...
static void setup_nway_xcvr(int ioaddr);
...
int do_update(int ioaddr, unsigned short *ee_values, int index,
			  char *field_name, int new_value);
static void check_for_intel_cb(int ioaddr, unsigned short *eeprom_contents);
...
#define EEPROM_SIZE 128			/* True size may be 512 for CardBus. */

/* Values read from the EEPROM, and the new image to write. */
unsigned short eeprom_contents[EEPROM_SIZE/2];
unsigned short new_ee_contents[EEPROM_SIZE/2];
...
int tulip_diag(int vendor_id, int device_id, long ioaddr, int part_idx)
{
	...
	if (flags & COMET_MACADDR) {
		printf(" The Comet MAC registers are %8.8x %8.8x filter %8.8x%8.8x.\n",
	...
	if (flags & DC21040_EEPROM) {
		...
		for (i = 0; i < sizeof(eeprom_contents)/2; i++)
	...
	if (show_eeprom || verbose > 1) {
		if (flags & DC21040_EEPROM)
			...
			liteon_eeprom((unsigned char *)eeprom_contents, part_idx);
		else
			parse_eeprom((unsigned char *)eeprom_contents, part_idx);
		if (show_eeprom > 1) {
	...
	for (media_idx = 0; media_idx < count; media_idx++) {
		if (flags & 0x01) {
			...
			case 4:
				...
				break;
			case 5:
				printf("   Adapter Reset Sequence, sequence length %d:", p[2]);
				for( i = 0; i < p[2]; i++)
