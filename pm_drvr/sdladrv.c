...
static DWORD sdla_memtest (sdlahw_t * hw);
static int sdla_bootcfg (sdlahw_t * hw, sfm_info_t * sfminfo);
static BYTE make_config_byte (sdlahw_t * hw);
...
static WORD checksum (BYTE * buf, unsigned len);
 ...
 */
static unsigned s502_port_options[] = { 4, 0x250, 0x300, 0x350, 0x360 }

        ;
static unsigned s503_port_options[] = { 8, 0x250, 0x254, 0x300, 0x304, 0x350, 0x354, 0x360, 0x364 }

        ;
static unsigned s508_port_options[] = { 8, 0x250, 0x270, 0x280, 0x300, 0x350, 0x360, 0x380, 0x390 }
...
int sdla_setup (sdlahw_t * hw, void *sfm, unsigned len)
{
  ...
  if (hw->dpmbase == 0)
  {
    ...
    if (err)
    {
      printk (KERN_ERR "%s: can't find available memory region!\n", modname);
      return err;
    }
  }
  ...
  {
    printk (KERN_ERR "%s: 8K memory region at 0x%lX is not available!\n", modname, hw->dpmbase);
    return -EINVAL;
  }
 ...
 */
int sdla_mapmem (sdlahw_t * hw, DWORD addr)
{
  unsigned port = hw->port;
  register int tmp;
 ...
 */
int sdla_peek (sdlahw_t * hw, DWORD addr, void *buf, unsigned len)
{
  DWORD oldvec = hw->vector;
  unsigned winsize = hw->dpmsize;
  unsigned curpos, curlen;	/* current offset and block size */
  DWORD curvec;			/* current DPM window vector */
  int err = 0;
 ...
 * interrupt routines are accessing adapter shared memory.
 */
int sdla_poke (sdlahw_t * hw, DWORD addr, void *buf, unsigned len)
{
  DWORD oldvec = hw->vector;
  unsigned winsize = hw->dpmsize;
  unsigned curpos, curlen;	/* current offset and block size */
...
static int sdla_load (sdlahw_t * hw, sfm_t * sfm, unsigned len)
{
  ...
  {
    printk (KERN_ERR "%s: firmware format %u rejected! Expecting %u.\n", modname, sfm->version, SFM_VERSION);
    return -EINVAL;
  }

  /* Verify firmware module length and checksum */
  if ((len - offsetof (sfm_t, image) != sfm->info.codesize) ||
      (checksum ((void *) &sfm->info, sizeof (sfm_info_t) + sfm->info.codesize) != sfm->checksum))
  {
  ...
  printk (KERN_INFO "%s: loading %s (ID=%u)...\n", modname,
	  (sfm->descr[0] != '\0') ? sfm->descr : "unknown firmware", sfm->info.codeid);

  /* Scan through the list of compatible adapters and make sure our
   * adapter type is listed.
   */
  for (i = 0; (i < SFM_MAX_SDLA) && (sfm->info.adapter[i] != hw->type); ++i)
    ;
  if (i == SFM_MAX_SDLA)
  {
  ...
  {
    printk (KERN_ERR "%s: firmware needs %lu bytes of on-board memory!\n", modname, sfm->info.memsize);
    return -EINVAL;
  }
...
static DWORD sdla_memtest (sdlahw_t * hw)
{
  ...
  for (memsize = 0, winsize = hw->dpmsize;
       !sdla_mapmem (hw, memsize) && (test_memregion ((void *) hw->dpmbase, winsize) == winsize); memsize += winsize)
    ;
  hw->memory = memsize;
  return memsize;
}
