...
static BYTE ni_vendor[] = { 0x02,0x07,0x01 };
...
static void ni65_init_lance(struct priv *p,BYTE*,int,int);
 ...
 */
static void ni65_init_lance(struct priv *p,BYTE *daddr,int filter,int mode)
{
  int i;
  u32 pib;
...
static int ni65_alloc_buffer(struct device *dev)
{
  ...
  p = dev->priv = (struct priv *) (((DWORD) ptr + 7) & ~0x7);
...
static void ni65_recv_intr(struct device *dev,int csr0)
{
  ...
  skb->dev = dev;
#ifdef RCV_VIA_SKB
        if( (DWORD) (skb->data + R_BUF_SIZE) > 0x1000000) {
          skb_put(skb,len);
          eth_copy_and_sum(skb, (BYTE *)(p->recv_skb[p->rmdnum]->data),len,0);
        }
        else {
...
#else
        skb_put(skb,len);
        eth_copy_and_sum(skb, (BYTE *) p->recvbounce[p->rmdnum],len,0);
#endif
        p->stats.rx_packets++;
...
#ifdef XMT_VIA_SKB
    if( (DWORD) (skb->data + skb->len) > 0x1000000) {
#endif
