...
struct slip {
  ...
  BYTE    *cbuff;    /* compression buffer    */
#endif

  /* These are pointers to the malloc()ed frame buffers. */
  BYTE    *rbuff;    /* receiver buffer    */
  int                   rcount;         /* received chars counter       */
  BYTE    *xbuff;    /* transmitter buffer    */
  BYTE         *xhead;         /* pointer to next byte to XMIT */
  ...
  DWORD    flags;    /* Flag values/ mode etc  */
...
#define SL_MODE_ADAPTIVE 8
  ...
  BYTE    outfill;  /* # of sec between outfill packet */
  BYTE    keepalive;  /* keepalive seconds    */
