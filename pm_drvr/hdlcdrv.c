...
static const WORD crc_ccitt_table[] = {
...
static inline void append_crc_ccitt(BYTE *buffer, int len)
{
   DWORD crc = 0xffff;
...
static inline int check_crc_ccitt(const BYTE *buf, int cnt)
{
  DWORD crc = 0xffff;
...
#if 0
static int calc_crc_ccitt(const BYTE *buf, int cnt)
{
  DWORD crc = 0xffff;
...
static int hdlc_rx_add_bytes(struct hdlcdrv_state *s, DWORD bits, 
           int num)
{
  int added = 0;
...
{
  int i;
  DWORD mask1, mask2, mask3, mask4, mask5, mask6, word;
...
static WORD random_seed;

static inline WORD random_num(void)
{
...
int hdlcdrv_register_hdlcdrv(struct device *dev, const struct hdlcdrv_ops *ops,
           DWORD privsize, char *ifname,
           DWORD baseaddr, DWORD irq, 
           DWORD dma) 
{
  struct hdlcdrv_state *s;
