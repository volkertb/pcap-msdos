...
struct mii_phy {
       ...
       BYTE  *gep;        /* Start of GEP sequence block in SROM */
       BYTE  *rst;        /* Start of reset sequence in SROM */
       DWORD  mc;         /* Media Capabilities */
       DWORD  ana;        /* NWay Advertisement */
       DWORD  fdx;        /* Full DupleX capabilites for each media */
       DWORD  ttm;        /* Transmit Threshold Mode for each media */
     };
...
#ifndef DE4X5_DEBUG
#define DE4X5_DEBUG 0
#endif
int de4x5_debug = DE4X5_DEBUG;
...
#define DE4X5_AUTOSENSE_MS 250  /* msec autosense tick (DE500) */
  ...
  static int de4x5_full_duplex = 1;
#else
  static int de4x5_full_duplex = 0;
#endif
...
/*#define DESC_ALIGN  DWORD dummy[4];  / * Must agree with DESC_SKIP_LEN */
#define DESC_ALIGN
...
struct de4x5_private {
       ...
       struct {
         DWORD  bins[DE4X5_PKT_STAT_SZ];       /* Private stats counters       */
         DWORD  unicast;
         DWORD  multicast;
         DWORD  broadcast;
       ...
       int  irq_mask;                /* Interrupt Mask (Enable) bits */
       int  irq_en;                  /* Summary interrupt bits       */
       ...
       struct {
         ...
         int   csr0;                 /* Saved Bus Mode Register      */
         int   csr6;                 /* Saved Operating Mode Reg.    */
         int   csr7;                 /* Saved IRQ Mask Register      */
         int   gep;                  /* Saved General Purpose Reg.   */
         int   gepc;                 /* Control info for GEP         */
         int   csr13;                /* Saved SIA Connectivity Reg.  */
         int   csr14;                /* Saved SIA TX/RX Register     */
         int   csr15;                /* Saved SIA General Register   */
       ...
       int    infoblock_csr6;        /* csr6 value in SROM infoblock */
       int    infoblock_media;       /* infoblock media              */
       int  (*infoleaf_fn)(DEVICE*); /* Pointer to infoleaf function */
       BYTE  *rst;                   /* Pointer to Type 5 reset info */
       BYTE   ibn;                   /* Infoblock number             */
     };
...
static void  de4x5_interrupt (int irq);
static int   de4x5_close (struct device *dev);
static void *de4x5_get_stats (struct device *dev);
...
static void load_packet (struct device *dev, char *buf, DWORD flags, struct sk_buff *skb);
...
static int  de4x5_suspect_state (struct device *dev, int timeout, int prev_state, int (*fn) (struct device *, int),
                                 int (*asfn) (struct device *));
static int  dc21040_state (struct device *dev, int csr13, int csr14, int csr15, int timeout, int next_state,
                           int suspect_state, int (*fn) (struct device *, int));
static int  test_media (struct device *dev, int irqs, int irq_mask, int csr13, int csr14, int csr15, int msec);
...
static void reset_init_sia (struct device *dev, int sicr, int strr, int sigr);
static int  test_ans (struct device *dev, int irqs, int irq_mask, int msec);
static int  test_tp (struct device *dev, int msec);
static int  EISA_signature (char *name, int eisa_id);
...
static short srom_rd (u_long address, BYTE offset);
static void srom_latch (DWORD command, u_long address);
static void srom_command (DWORD command, u_long address);
static void srom_address (DWORD command, u_long address, BYTE offset);
static short srom_data (DWORD command, u_long address);

static void sendto_srom (DWORD command, u_long addr);
...
static void srom_exec (struct device *dev, BYTE * p);
static int  mii_rd (BYTE phyreg, BYTE phyaddr, u_long ioaddr);
static void mii_wr (int data, BYTE phyreg, BYTE phyaddr, u_long ioaddr);
...
static void enable_ast (struct device *dev, DWORD time_out);
...
static int type0_infoblock (struct device *dev, BYTE count, BYTE * p);
static int type1_infoblock (struct device *dev, BYTE count, BYTE * p);
static int type2_infoblock (struct device *dev, BYTE count, BYTE * p);
static int type3_infoblock (struct device *dev, BYTE count, BYTE * p);
static int type4_infoblock (struct device *dev, BYTE count, BYTE * p);
static int type5_infoblock (struct device *dev, BYTE count, BYTE * p);
static int compact_infoblock (struct device *dev, BYTE count, BYTE * p);

static int autoprobed = 0, loading_module = 0;

static char name[DE4X5_NAME_LENGTH + 1];
static BYTE de4x5_irq[] = EISA_ALLOWED_IRQ_LIST;
 ...
 */
static int (*dc_infoblock[]) (struct device *dev, BYTE, BYTE*) = {
           type0_infoblock,
           type1_infoblock,
           type2_infoblock,
           type3_infoblock,
 ...
 * ** more info. Until I fix (un)register_netdevice() we won't be able to
 * ** use it though.
 */
...
static int de4x5_hw_init (struct device *dev, u_long iobase)
{
  ...
  else pcibios_write_config_byte (lp->bus_num, lp->device << 3,
                                  PCI_CFDA_PSM, WAKEUP);
  de4x5_ms_delay (10);
  ...
  if ((inl (DE4X5_STS) & (STS_TS | STS_RS)) != 0)
     return (0);              /* Hardware could not reset */
  ...
  else EISA_signature (name, EISA_ID0);
  ...
  else printk ("%s: %s at 0x%04lx (PCI bus %d, device %d)",
               dev->name, name, iobase, lp->bus_num, lp->device);
   ...
   */
  dev->priv = k_malloc (sizeof(struct de4x5_private) + ALIGN);
  if (!dev->priv)
     return (0);
  ...
  priv = (struct de4x5_private *) dev->priv;
  memset (dev->priv, 0, sizeof (struct de4x5_private));

  priv->bus     = bus.bus;
  priv->bus_num = bus.bus_num;
  priv->device  = bus.device;
  priv->chipset = bus.chipset;
  priv->cache.priv = tmp;
  priv->cache.gepc = GEP_INIT;
  priv->timeout = -1;
  priv->useSROM = useSROM;
  memcpy (&priv->srom, &bus.srom, sizeof(struct de4x5_srom));

  /*
   * ** Choose correct autosensing in case someone messed up
   */
  if ((de4x5_autosense & AUTO) || priv->useSROM)
     priv->autosense = AUTO;
  else
  {
    if (priv->chipset != DC21140)
    {
      if ((priv->chipset == DC21040) && (de4x5_autosense & TP_NW))
         de4x5_autosense = TP;
      if ((priv->chipset == DC21041) && (de4x5_autosense & BNC_AUI))
         de4x5_autosense = BNC;
      priv->autosense = de4x5_autosense & 0x001f;
    }
    else
      priv->autosense = de4x5_autosense & 0x00c0;
  }
  priv->fdx = de4x5_full_duplex;
  sprintf (priv->adapter_name, "%s (%s)", name, dev->name);

  /*
   * ** Set up the RX descriptor ring (Intels)
   * ** Allocate contiguous receive buffers, long word aligned (Alphas)
   */
#if !defined(DE4X5_DO_MEMCPY)
  for (i = 0; i < NUM_RX_DESC; i++)
  {
    priv->rx_ring[i].status = 0;
    priv->rx_ring[i].des1 = RX_BUFF_SZ;
    priv->rx_ring[i].buf = 0;
    priv->rx_ring[i].next = 0;
    priv->rx_skb[i] = (struct sk_buff *) 1;     /* Dummy entry */
  }

#else
  if ((tmp = k_malloc (RX_BUFF_SZ * NUM_RX_DESC + ALIGN)) == NULL)
  {
    k_free (priv->cache.priv);
    return (0);
  }
  ...
  for (i = 0; i < NUM_RX_DESC; i++)
  {
    ...
    priv->rx_ring[i].buf    = cpu_to_le32 (virt_to_bus (tmp + i * RX_BUFF_SZ));
    priv->rx_ring[i].next   = 0;
    priv->rx_skb[i]         = (struct sk_buff*) 1;     /* Dummy entry */
  }
#endif
  ...
  priv->rxRingSize = NUM_RX_DESC;
  priv->txRingSize = NUM_TX_DESC;

  /* Write the end of list marker to the descriptor lists
   */
  priv->rx_ring[priv->rxRingSize - 1].des1 |= cpu_to_le32 (RD_RER);
  priv->tx_ring[priv->txRingSize - 1].des1 |= cpu_to_le32 (TD_TER);

  /* Tell the adapter where the TX/RX rings are located.
   */
  outl (virt_to_bus (priv->rx_ring), DE4X5_RRBA);
  outl (virt_to_bus (priv->tx_ring), DE4X5_TRBA);

  /* Initialise the IRQ mask and Enable/Disable
   */
  priv->irq_mask = IMR_RIM | IMR_TIM | IMR_TUM | IMR_UNM;
  priv->irq_en   = IMR_NIM | IMR_AIM;

  /* Create a loopback packet frame for later media probing
   */
  create_packet (dev, priv->frame, sizeof (priv->frame));

  /* Check if the RX overflow bug needs testing for
   */
  i = cfrv & 0x000000fe;
  if ((priv->chipset == DC21140) && (i == 0x20))
    priv->rx_ovf = 1;
  ...
  if (priv->useSROM)
  {
    priv->state = INITIALISED;
    de4x5_dbg_srom ((struct de4x5_srom*)&priv->srom);
   ...
   */
  if ((priv->chipset != DC21040) && (priv->chipset != DC21041))
     mii_get_phy (dev);

  printk ("      and requires IRQ%d (provided by %s).\n",
          dev->irq, lp->bus == PCI ? "PCI BIOS" : "EISA CNFG");
   ...
   */
  dev->open = de4x5_open;
  dev->xmit = de4x5_queue_pkt;
  dev->close = de4x5_close;
...
static int de4x5_sw_reset (struct device *dev)
{
  ...
  if (!lp->useSROM)
  {
    ...
    else lp->infoblock_csr6 = OMR_TTM;
   ...
   */
  ENABLE();                   /* Ensure timer interrupts */
  for (j = 0, i = 0; (i < 500) && (j == 0); i++)
  {				/* Upto 500ms delay */
    udelay (1000);
    if (le32_to_cpu (lp->tx_ring[lp->tx_new].status) >= 0)
       j = 1;
  }
...
static int de4x5_queue_pkt (struct sk_buff *skb, struct device *dev)
{
  ...
  set_bit (0, (void*)&dev->tx_busy);  /* Stop send re-tries */
   ...
   */
  if (set_bit (0, (void*)&lp->cache.lock) && !dev->reentry)
     return (0);

  /* Transmit descriptor ring full or stale skb
   */
  if (dev->tx_busy || lp->tx_skb[lp->tx_new])
  {
    if (dev->reentry)
         de4x5_putb_cache (dev, skb);      /* Requeue the buffer */
    else de4x5_put_cache (dev, skb);
    ...
    if (de4x5_debug & DEBUG_TX)
       printk ("%s: transmit busy, lost media or stale skb found:\n"
               ...
               "  OMR:%08x\n"
               "  Stale skb: %s\n",
               dev->name, inl (DE4X5_STS), dev->tx_busy, inl (DE4X5_IMR),
               inl (DE4X5_OMR), lp->tx_skb[lp->tx_new] ? "YES" : "NO");
  }
  else if (skb->len > 0)
  {
    /* If we already have stuff queued locally, use that first
     */
    if (lp->cache.skb && !dev->reentry)
    {
      de4x5_put_cache (dev, skb);
      skb = de4x5_get_cache (dev);
    }

    while (skb && !dev->tx_busy && !lp->tx_skb[lp->tx_new])
    {
      cli();
      set_bit (0, (void *) &dev->tx_busy);
      ...
      if (TX_BUFFS_AVAIL)
         dev->tx_busy = 0;       /* Another pkt may be queued */
      skb = de4x5_get_cache (dev);
      sti();
    }
    if (skb)
...
static void de4x5_interrupt (int irq)
{
  ...
  {
    while (lp->cache.skb && !dev->tx_busy && lp->tx_enable)
...
static int de4x5_rx (struct device *dev)
{
  ...
  for (entry = lp->rx_new; le32_to_cpu (lp->rx_ring[entry].status) >= 0;
       entry = lp->rx_new)
  {
...
static int de4x5_tx (struct device *dev)
{
  ...
  for (entry = lp->tx_old; entry != lp->tx_new; entry = lp->tx_old)
  {
	...
	if (lp->tx_enable)
      ...
      lp->stats.tx_collisions += ((status & TD_EC) ? 16 :
                                 ((status & TD_CC) >> 3));
...
static void de4x5_local_stats (struct device *dev, char *buf, int pkt_len)
{
  ...
  else if ((*(s32 *) & buf[0] == *(s32 *) & dev->dev_addr[0]) && (*(s16 *) & buf[4] == *(s16 *) & dev->dev_addr[4]))
  {
    lp->pktStats.unicast++;
  }
...
static void set_multicast_list (struct device *dev)
{
  ...
  if (lp->state == OPEN)
  {
    ...
    {
      SetMulticastFilter (dev);
      load_packet (dev, lp->setup_frame, TD_IC | PERFECT_F | TD_SET | SETUP_FRAME_LEN, NULL);
...
static void SetMulticastFilter (struct device *dev)
{
  ...
  int i, j, bit, byte;
  u16 hashcode;
  DWORD omr, crc, poly = CRC_POLYNOMIAL_LE;
  char *pa;
  BYTE *addrs;
...
static void pci_probe (struct device *dev, DWORD ioaddr)
{
  BYTE irq;
  BYTE pb, pbus, dev_num, dnum, dev_fn;
  u_short vendor, index, status;
  DWORD class = DE4X5_CLASS_CODE;
  DWORD device, iobase;
  struct bus_type *lp = &bus;
  struct device *tmp;
  ...
  for (index = 0; (pcibios_find_class (class, index, &pb, &dev_fn) != PCIBIOS_DEVICE_NOT_FOUND); index++)
  {
	...
	device |= (cfrv & 0x00f0);
      ...
      {
	printk
	  ("de4x5: Detected a %s chip. Currently this is unsupported in this driver.\nPlease email the author to request its inclusion!\n",
...
static struct device *alloc_device (struct device *dev, DWORD iobase)
{
  ...
  if (((dev->next == NULL) && ((dev->base_addr != DE4X5_NDA) && (dev->base_addr != 0)) && !fixed) || new_dev)
  {
    num_eth++;			/* New device */
    dev = insert_device (dev, iobase, de4x5_probe);
  }
...
static struct device *unlink_modules (struct device *p)
{
  ...
  {				/* Private areas allocated?  */
    ...
    release_region (p->base_addr, (lp->bus == PCI ? DE4X5_PCI_TOTAL_SIZE : DE4X5_EISA_TOTAL_SIZE));
  }
  unregister_netdev (p);
...
static int dc21040_autoconf (struct device *dev)
{
  ...
  switch (lp->media)
  {
	 ...
	 break;
       ...
       case TP:
	 next_tick = dc21040_state (dev, 0x8f01, 0xffff, 0x0000, 3000, BNC_AUI, TP_SUSPECT, test_tp);
	 break;
	 ...
	 break;
       ...
       case AUI:
       case BNC_AUI:
	 next_tick = dc21040_state (dev, 0x8f09, 0x0705, 0x0006, 3000, EXT_SIA, BNC_AUI_SUSPECT, ping_media);
	 break;
	 ...
	 break;
       ...
       case EXT_SIA:
	 next_tick = dc21040_state (dev, 0x3041, 0x0000, 0x0006, 3000, NC, EXT_SIA_SUSPECT, ping_media);
	 break;
...
dc21040_state (struct device *dev, int csr13, int csr14, int csr15, int timeout,
	       int next_state, int suspect_state, int (*fn) (struct device *, int))
{
  struct de4x5_private *lp = (struct de4x5_private *) dev->priv;
  int next_tick = DE4X5_AUTOSENSE_MS;
  int linkBad;
...
de4x5_suspect_state (struct device *dev, int timeout, int prev_state,
		     int (*fn) (struct device *, int), int (*asfn) (struct device *))
{
  struct de4x5_private *lp = (struct de4x5_private *) dev->priv;
  int next_tick = DE4X5_AUTOSENSE_MS;
  int linkBad;
  ...
  switch (lp->media)
  {
	 ...
	 else
	 {
	   ...
	   else
	   {
	     ...
	     else if ((lp->autosense == AUTO) && ((sr = is_anc_capable (dev)) & MII_SR_ANC))
	     {
	 ...
	 switch (lp->local_state)
	 {
		...
		else
		{
		  ...
		  {		/* Success! */
		    ...
		    if (!(anlpa & MII_ANLPA_RF) && (cap = anlpa & MII_ANLPA_TAF & ana))
		    {
		      if (cap & MII_ANA_100M)
		      {
	 ...
	 if (lp->timeout < 0)
	 {
	   lp->tmp = (lp->phy[lp->active].id ? MII_SR_LKS : (~inl (DE4X5_GEP) & GEP_LNP));
	   SET_100Mb_PDET;
	 }
...
static int test_mii_reg (struct device *dev, int reg, int mask, int pol, long msec)
{
  ...
  if (pol)
    pol = ~0;
  reg = mii_rd ((BYTE) reg, lp->phy[lp->active].addr, DE4X5_MII) & mask;
  test = (reg ^ pol) & mask;
...
static int is_spd_100 (struct device *dev)
{
  ...
  {
    spd = (lp->asBitValid & (lp->asPolarity ^ (inl (DE4X5_GEP) & lp->asBit))) | (lp->linkOK & ~lp->asBitValid);
  }
...
static int is_100_up (struct device *dev)
{
  ...
  if (lp->useSROM && !lp->useMII)
  {
    return ((lp->asBitValid & (lp->asPolarity ^ (inl (DE4X5_GEP) & lp->asBit))) | (lp->linkOK & ~lp->asBitValid));
  }
  else if (lp->phy[lp->active].id && (!lp->useSROM || lp->useMII))
  {
...
static int ping_media (struct device *dev, int msec)
{
  ...
  if ((!(sisr & SISR_NCR)) && ((s32) le32_to_cpu (lp->tx_ring[lp->tmp].status) < 0) && (--lp->timeout))
  {
    sisr = 100 | TIMER_CB;
  }
  else
  {
    if ((!(sisr & SISR_NCR)) && !(le32_to_cpu (lp->tx_ring[lp->tmp].status) & (T_OWN | TD_ES)) && lp->timeout)
    {
      sisr = 0;
    }
    else
...
static struct sk_buff *de4x5_alloc_rx_buff (struct device *dev, int index, int len)
{
  ...
  {				/* Wrapped buffer */
    ...
    memcpy (skb_put (p, tlen), bus_to_virt (le32_to_cpu (lp->rx_ring[lp->rx_old].buf)), tlen);
    memcpy (skb_put (p, len - tlen), bus_to_virt (le32_to_cpu (lp->rx_ring[0].buf)), len - tlen);
  }
  else
  {				/* Linear buffer */
    memcpy (skb_put (p, len), bus_to_virt (le32_to_cpu (lp->rx_ring[lp->rx_old].buf)), len);
  }
 ...
 */
static void DevicePresent (DWORD aprom_addr)
{
  int i;
  struct bus_type *lp = &bus;
...
static int get_hw_addr (struct device *dev)
{
  ...
  for (i = 0, k = 0, j = 0; j < 3; j++)
  {
	...
	k += (u_short) (tmp << 8);
      ...
      {
        dev->dev_addr[i] = (BYTE) lp->srom.ieee_addr[i];
	i++;
        dev->dev_addr[i] = (BYTE) lp->srom.ieee_addr[i];
	i++;
      }
...
static int test_bad_enet (struct device *dev, int status)
{
  ...
  if ((tmp == 0) || (tmp == 0x5fa))
  {
	...
	last.addr[i] = dev->dev_addr[i];
      if (((*((int *) dev->dev_addr) & 0x00ffffff) == 0x95c000) && (lp->chipset == DC21040))
      {
	dev->irq = last.irq;
      }
      status = 0;
 ...
 * ** SROM Read
 */
static short srom_rd (DWORD addr, BYTE offset)
{
...
static void srom_latch (DWORD command, DWORD addr)
{
...
static void srom_command (DWORD command, DWORD addr)
{
...
static void srom_address (DWORD command, DWORD addr, BYTE offset)
{
  int i;
  char a;
...
static short srom_data (DWORD command, DWORD addr)
{
  int i;
  short word = 0;
  int tmp;
...
#if 0
static void srom_busy (DWORD command, DWORD addr)
{
...
static void sendto_srom (DWORD command, DWORD addr)
{
  outl (command, addr);
  udelay (1);
}

static int getfrom_srom (DWORD addr)
{
  int tmp;
...
static void srom_init (struct device *dev)
{
  struct de4x5_private *lp = (struct de4x5_private *) dev->priv;
  DWORD iobase = dev->base_addr;
  BYTE *p = (BYTE *) & lp->srom + lp->infoleaf_offset;
  BYTE count;
...
static int type0_infoblock (struct device *dev, BYTE count, BYTE * p)
{
  struct de4x5_private *lp = (struct de4x5_private *) dev->priv;
  DWORD iobase = dev->base_addr;
  BYTE flags, csr6, len = (*p & BLOCK_LEN) + 1;
...
static int type1_infoblock (struct device *dev, BYTE count, BYTE * p)
{
  ...
  if (lp->state == INITIALISED)
  {
    ...
    p += (*p + 1);
    lp->phy[lp->active].mc = *(u_short *) p;
    p += 2;
    lp->phy[lp->active].ana = *(u_short *) p;
    p += 2;
    lp->phy[lp->active].fdx = *(u_short *) p;
    p += 2;
...
static int mii_rd (BYTE phyreg, BYTE phyaddr, DWORD ioaddr)
{
...
static void mii_wr (int data, BYTE phyreg, BYTE phyaddr, DWORD ioaddr)
{
  mii_wdata (MII_PREAMBLE, 2, ioaddr);	/* Start of 34 bit preamble...    */
  mii_wdata (MII_PREAMBLE, 32, ioaddr);	/* ...continued                   */
...
static int mii_rdata (DWORD ioaddr)
{
  int i;
  int tmp = 0;
...
static void mii_wdata (int data, int len, DWORD ioaddr)
{
  int i;
...
static void mii_address (BYTE addr, DWORD ioaddr)
{
  int i;
...
static void mii_ta (DWORD rw, DWORD ioaddr)
{
  if (rw == MII_STWR)
  {
...
static void sendto_mii (u32 command, int data, DWORD ioaddr)
{
  DWORD j;
...
static int getfrom_mii (u32 command, DWORD ioaddr)
{
  outl (command, ioaddr);
  udelay (1);
 ...
 */
static int mii_get_oui (BYTE phyaddr, DWORD ioaddr)
{
  /*
   * union {
   * u_short reg;
...
/*    return ((r2<<6)|(DWORD)(r3>>10)); *//* NATIONAL and BROADCOM way */
...
static int mii_get_phy (struct device *dev)
{
  ...
  for (n = 0, lp->mii_cnt = 0, i = 1; !((i == 1) && (n == 1)); i = (++i) % DE4X5_MAX_MII)
  {
	...
	continue;		/* ID match? */
      ...
      if (k < DE4X5_MAX_PHY)
      {
	memcpy ((char *) &lp->phy[k], (char *) &phy_info[j], sizeof (struct phy_table));
...
static void enable_ast (struct device *dev, DWORD time_out)
{
  timeout (dev, (void *) &de4x5_ast, (DWORD) dev, time_out);
...
static void timeout (struct device *dev, void (*fn) (DWORD data), DWORD data, DWORD msec)
{
  struct de4x5_private *lp = (struct de4x5_private *) dev->priv;
  int dt;
...
static void yawn (struct device *dev, int state)
{
  ...
  else
  {
    ...
    {
	 case WAKEUP:
	   pcibios_write_config_byte (lp->bus_num, lp->device << 3, PCI_CFDA_PSM, WAKEUP);
	   de4x5_ms_delay (10);
	   break;

	 case SNOOZE:
	   pcibios_write_config_byte (lp->bus_num, lp->device << 3, PCI_CFDA_PSM, SNOOZE);
	   break;

	 case SLEEP:
	   outl (0, DE4X5_SICR);
	   pcibios_write_config_byte (lp->bus_num, lp->device << 3, PCI_CFDA_PSM, SLEEP);
	   break;
    }
  }
...
static void de4x5_dbg_open (struct device *dev)
{
  ...
  if (de4x5_debug & DEBUG_OPEN)
  {
    ...
    for (i = 0; i < lp->rxRingSize - 1; i++)
    {
      if (i < 3)
      {
        printk ("0x%8.8lx  ", (DWORD) & lp->rx_ring[i].status);
      }
    }
    printk ("...0x%8.8lx\n", (DWORD) & lp->rx_ring[i].status);
    printk ("TX: ");
    for (i = 0; i < lp->txRingSize - 1; i++)
    {
      if (i < 3)
      {
        printk ("0x%8.8lx  ", (DWORD) & lp->tx_ring[i].status);
      }
    }
    printk ("...0x%8.8lx\n", (DWORD) & lp->tx_ring[i].status);
    ...
    printk ("Ring size: \nRX: %d\nTX: %d\n", (short) lp->rxRingSize, (short) lp->txRingSize);
  }
		...
		(lp->media == NC ? "unconnected!" :
		 (lp->media == TP ? "TP." :
		  (lp->media == ANS ? "TP/Nway." :
		   (lp->media == BNC ? "BNC." :
		    (lp->media == AUI ? "AUI." :
		     (lp->media == BNC_AUI ? "BNC/AUI." : (lp->media == EXT_SIA ? "EXT SIA." : "???."))))))));
      }
      else
      {
	printk ("%s: mode is %s\n", dev->name,
		(lp->media == NC ? "link down or incompatible connection." :
		 (lp->media == _100Mb ? "100Mb/s." : (lp->media == _10Mb ? "10Mb/s." : "\?\?\?"))));
      }
    }
    lp->c_media = lp->media;
  }
...
static void de4x5_dbg_srom (struct de4x5_srom *p)
{
  ...
  if (de4x5_debug & DEBUG_SROM)
  {
    ...
    printk ("# controllers:         %02x\n", (BYTE) (p->num_controllers));

    printk ("Hardware Address:     ");
    for (i = 0; i < ETH_ALEN - 1; i++)
    {
      printk ("%02x:", (BYTE) * (p->ieee_addr + i));
    }
    printk ("%02x\n", (BYTE) * (p->ieee_addr + i));
...
static void de4x5_dbg_rx (struct sk_buff *skb, int len)
{
  ...
  if (de4x5_debug & DEBUG_RX)
  {
    printk ("R: %02x:%02x:%02x:%02x:%02x:%02x <- %02x:%02x:%02x:%02x:%02x:%02x len/SAP:%02x%02x [%d]\n",
            ...
            (BYTE) skb->data[8],
            (BYTE) skb->data[9],
            (BYTE) skb->data[10], (BYTE) skb->data[11], (BYTE) skb->data[12], (BYTE) skb->data[13], len);
    if (de4x5_debug & DEBUG_RX)
    {
...
static int de4x5_ioctl (struct device *dev, struct ifreq *rq, int cmd)
{
  ...
  switch (ioc->cmd)
  {
	 ...
	 while (set_bit (0, (void *) &dev->tx_busy) != 0) ;	/* Wait for lock to free */
	 load_packet (dev, lp->setup_frame, TD_IC | PERFECT_F | TD_SET | SETUP_FRAME_LEN, NULL);
	 lp->tx_new = (++lp->tx_new) % lp->txRingSize;
	 outl (POLL_DEMAND, DE4X5_TPD);	/* Start the TX */
...
#define DE4X5_DUMP              0x0f	/* Dump the DE4X5 Status */
	 ...
	 tmp.lval[j >> 2] = (long) lp->rx_ring;
	 j += 4;
	 tmp.lval[j >> 2] = (long) lp->tx_ring;
	 j += 4;
	 ...
	 for (i = 0; i < lp->rxRingSize - 1; i++)
	 {
	   if (i < 3)
	   {
	     tmp.lval[j >> 2] = (long) &lp->rx_ring[i].status;
	     j += 4;
	   }
	 }
	 tmp.lval[j >> 2] = (long) &lp->rx_ring[i].status;
	 ...
	 for (i = 0; i < lp->txRingSize - 1; i++)
	 {
	   if (i < 3)
	   {
	     tmp.lval[j >> 2] = (long) &lp->tx_ring[i].status;
	     j += 4;
	   }
	 }
	 tmp.lval[j >> 2] = (long) &lp->tx_ring[i].status;
	 ...
	 for (i = 0; i < lp->rxRingSize - 1; i++)
	 {
	   if (i < 3)
	   {
	     tmp.lval[j >> 2] = (s32) le32_to_cpu (lp->rx_ring[i].buf);
	     j += 4;
	   }
	 }
	 tmp.lval[j >> 2] = (s32) le32_to_cpu (lp->rx_ring[i].buf);
	 ...
	 for (i = 0; i < lp->txRingSize - 1; i++)
	 {
	   if (i < 3)
	   {
	     tmp.lval[j >> 2] = (s32) le32_to_cpu (lp->tx_ring[i].buf);
	     j += 4;
	   }
	 }
	 tmp.lval[j >> 2] = (s32) le32_to_cpu (lp->tx_ring[i].buf);
	 ...
	 for (i = 0; i < lp->rxRingSize; i++)
	 {
	   tmp.lval[j >> 2] = le32_to_cpu (lp->rx_ring[i].status);
	   j += 4;
	 }
	 for (i = 0; i < lp->txRingSize; i++)
	 {
	   tmp.lval[j >> 2] = le32_to_cpu (lp->tx_ring[i].status);
	   j += 4;
	 }

	 tmp.lval[j >> 2] = inl (DE4X5_BMR);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_TPD);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_RPD);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_RRBA);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_TRBA);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_STS);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_OMR);
	 j += 4;
	 tmp.lval[j >> 2] = inl (DE4X5_IMR);
	 j += 4;
	 tmp.lval[j >> 2] = lp->chipset;
	 j += 4;
	 if (lp->chipset == DC21140)
	 {
	   tmp.lval[j >> 2] = inl (DE4X5_GEP);
	   j += 4;
	 }
	 else
	 {
	   tmp.lval[j >> 2] = inl (DE4X5_SISR);
	   j += 4;
	   tmp.lval[j >> 2] = inl (DE4X5_SICR);
	   j += 4;
	   tmp.lval[j >> 2] = inl (DE4X5_STRR);
	   j += 4;
	   tmp.lval[j >> 2] = inl (DE4X5_SIGR);
	   j += 4;
	 }
	 tmp.lval[j >> 2] = lp->phy[lp->active].id;
	 j += 4;
	 if (lp->phy[lp->active].id && (!lp->useSROM || lp->useMII))
	 {
	   tmp.lval[j >> 2] = lp->active;
	   j += 4;
	   tmp.lval[j >> 2] = mii_rd (MII_CR, lp->phy[lp->active].addr, DE4X5_MII);
	   j += 4;
	   tmp.lval[j >> 2] = mii_rd (MII_SR, lp->phy[lp->active].addr, DE4X5_MII);
	   j += 4;
	   tmp.lval[j >> 2] = mii_rd (MII_ID0, lp->phy[lp->active].addr, DE4X5_MII);
	   j += 4;
	   tmp.lval[j >> 2] = mii_rd (MII_ID1, lp->phy[lp->active].addr, DE4X5_MII);
	   j += 4;
	   if (lp->phy[lp->active].id != BROADCOM_T4)
	   {
	     tmp.lval[j >> 2] = mii_rd (MII_ANA, lp->phy[lp->active].addr, DE4X5_MII);
	     j += 4;
	     tmp.lval[j >> 2] = mii_rd (MII_ANLPA, lp->phy[lp->active].addr, DE4X5_MII);
	     j += 4;
	   }
	   tmp.lval[j >> 2] = mii_rd (0x10, lp->phy[lp->active].addr, DE4X5_MII);
	   j += 4;
	   if (lp->phy[lp->active].id != BROADCOM_T4)
	   {
	     tmp.lval[j >> 2] = mii_rd (0x11, lp->phy[lp->active].addr, DE4X5_MII);
	     j += 4;
	     tmp.lval[j >> 2] = mii_rd (0x12, lp->phy[lp->active].addr, DE4X5_MII);
	     j += 4;
	   }
	   else
	   {
	     tmp.lval[j >> 2] = mii_rd (0x14, lp->phy[lp->active].addr, DE4X5_MII);
	     j += 4;
	   }
	 }

	 tmp.addr[j++] = lp->txRingSize;
	 tmp.addr[j++] = dev->tx_busy;
