...
static void happy_meal_timer(DWORD data)
{
            ...
            struct hmeal_bigmacregs *bregs)
{
  ...
  stats->tx_collisions += (bregs->ex_ctr + bregs->lt_ctr);
...
static void happy_meal_init_rings(struct happy_meal *hp, int from_irq)
{
  ...
  for(i = 0; i < RX_RING_SIZE; i++) {
    ...
    hb->happy_meal_rxd[i].rx_addr = (DWORD) skb->data;
...
static void sun4c_happy_meal_init_rings(struct happy_meal *hp)
{
  ...
  HMD(("init rxring, "));
  for(i = 0; i < RX_RING_SIZE; i++) {
    BYTE *this_buf = &hbufs->rx_buf[i][0];

    hb->happy_meal_rxd[i].rx_addr = (DWORD) this_buf;
    hb->happy_meal_rxd[i].rx_flags =
...
static int happy_meal_init(struct happy_meal *hp, int from_irq)
{
  ...
  HMD(("ring ptrs\n"));
  erxregs->rx_ring = (DWORD) &hp->happy_block->happy_meal_rxd[0];
  etxregs->tx_ring = (DWORD) &hp->happy_block->happy_meal_txd[0];
         ...
         struct hmeal_gregs *gregs)
{
  ...
  drop_it:
      ...
      this->rx_addr = (DWORD) hp->rx_skbs[elem]->data;
      this->rx_flags =
        (RXFLAG_OWN | ((RX_BUF_ALLOC_SIZE - RX_OFFSET) << 16));
      goto next;
    }
               ...
               struct hmeal_gregs *gregs)
{
  ...
  while(!(this->rx_flags & RXFLAG_OWN)) {
    struct sk_buff *skb;
    DWORD flags = this->rx_flags;
    BYTE *thisbuf = &hbufs->rx_buf[elem][0];
    int len = flags >> 16;
...
static int happy_meal_start_xmit(struct sk_buff *skb, struct device *dev)
{
  ...
  hp->happy_block->happy_meal_txd[entry].tx_addr = (DWORD) skb->data;
...
static int sun4c_happy_meal_start_xmit(struct sk_buff *skb, struct device *dev)
{
  ...
  SXD(("SX<l[%d]e[%d]>", len, entry));
  hp->happy_block->happy_meal_txd[entry].tx_addr = (DWORD) txbuf;
  hp->happy_block->happy_meal_txd[entry].tx_flags =
    (TXFLAG_OWN | TXFLAG_SOP | TXFLAG_EOP | (len & TXFLAG_SIZE));
  hp->tx_new = NEXT_TX(entry);
