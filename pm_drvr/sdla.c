...
static DWORD valid_port[] = {
             0x250, 0x270, 0x280, 0x300, 0x350, 0x360, 0x380, 0x390
           };

static DWORD valid_mem[] = {
             0xA0000, 0xA2000, 0xA4000, 0xA6000, 0xA8000, 0xAA000, 0xAC000,
             0xAE000, 0xB0000, 0xB2000, 0xB4000, 0xB6000, 0xB8000, 0xBA000,
             0xBC000, 0xBE000, 0xC0000, 0xC2000, 0xC4000, 0xC6000, 0xC8000,
             0xCA000, 0xCC000, 0xCE000, 0xD0000, 0xD2000, 0xD4000, 0xD6000,
             0xD8000, 0xDA000, 0xDC000, 0xDE000, 0xE0000, 0xE2000, 0xE4000,
             0xE6000, 0xE8000, 0xEA000, 0xEC000, 0xEE000
           };

/*
 * these are the core routines that access the card itself
 */
#define SDLA_WINDOW(dev,addr) \
        outb ((((addr) >> 13) & 0x1F), (dev)->base_addr + SDLA_REG_Z80_WINDOW)
...
static void sdla_clear (struct device *dev)
{
  int   len   = 65336;
  int   addr  = 0;
  int   bytes = SDLA_WINDOW_SIZE;
  char *base  = (void*) dev->mem_start;
...
int sdla_dlci_conf (struct device *slave, struct device *master, int get)
{
  ...
  if (slave->start)
    if (get)
      ret = sdla_cmd (slave, SDLA_READ_DLCI_CONFIGURATION, abs (flp->dlci[i]), 0, NULL, 0, &dlp->config, &len);
    else
  ...
  else
  {
    ...
    if (accept)
    {
      ...
      switch (flp->type)
      {
	   ...
	   case SDLA_S502E:
	     ret =
	       sdla_cmd (dev, SDLA_INFORMATION_WRITE, *(short *) (skb->dev->dev_addr), 0, skb->data, skb->len, NULL,
			 NULL);
	     break;
	   ...
	   case SDLA_S508:
	     size = sizeof (addr);
	     ret =
	       sdla_cmd (dev, SDLA_INFORMATION_WRITE, *(short *) (skb->dev->dev_addr), 0, NULL, skb->len, &addr, &size);
	     if (ret == SDLA_RET_OK)
	     {
	       save_flags (flags);
	       cli ();
...
static void sdla_receive (struct device *dev)
{
  ...
  switch (flp->type)
  {
	 ...
	 pbuf =
	   ...
	   (void *) (dev->mem_start + ((pbufi->rse_base + flp->buffer * sizeof (struct buf_entry)) & SDLA_ADDR_MASK));
...
static void sdla_poll (DWORD device)
{
  struct device *dev;
  struct frad_local *flp;
...
static int sdla_open (struct device *dev)
{
  ...
  for (i = 0; i < CONFIG_DLCI_MAX; i++)
    if (flp->dlci[i])
    {
      ...
      if (dlp->configured)
	sdla_cmd (dev, SDLA_SET_DLCI_CONFIGURATION, abs (flp->dlci[i]), 0, &dlp->config, sizeof (struct dlci_conf),
		  NULL, NULL);
    }
...
static int sdla_config (struct device *dev, struct frad_conf *conf, int get)
{
  ...
  if (!get)
  {
    ...
    if ((data.config.kbaud < 0) || ((data.config.kbaud > 128) && (flp->type != SDLA_S508)))
      return (-EINVAL);
