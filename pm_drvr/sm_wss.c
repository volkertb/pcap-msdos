...
static void write_codec(struct device *dev, BYTE idx,
      BYTE data)
{
  int timeout = 900000;
...
static BYTE read_codec(struct device *dev, BYTE idx)
{
  int timeout = 900000;
...
static int wss_set_codec_fmt(struct device *dev, struct sm_state *sm, 
           BYTE fmt, char fdx, char fullcalib)
{
  unsigned long time;
  unsigned long flags;
...
static int wss_init_codec(struct device *dev, struct sm_state *sm, char fdx, 
        BYTE src_l, BYTE src_r, 
        int igain_l, int igain_r,
        int ogain_l, int ogain_r)
{
  BYTE tmp, reg0, reg1, reg6, reg7;
...
{
        unsigned long flags;
        static const BYTE codecmode[2] = { 0x0e, 0x0d };
        static const BYTE dmamode[2] = { 
