...
static const char __initdata *version = "acenic.c: v0.34 09/03/99  "
...
int __init acenic_probe (void)
{
  ...
  while ((pdev = pci_find_class (PCI_CLASS_NETWORK_ETHERNET << 8, pdev)))
  {
    ...
    if (!((pdev->vendor == PCI_VENDOR_ID_ALTEON) &&
        ...
        !((pdev->vendor == PCI_VENDOR_ID_NETGEAR) && (pdev->device == PCI_DEVICE_ID_NETGEAR_GA620)) &&
        /*
        ...
        !((pdev->vendor == PCI_VENDOR_ID_SGI) && (pdev->device == PCI_DEVICE_ID_SGI_ACENIC)))
      continue;
    ...
    if (dev == NULL)
    {
      printk ("acenic: Unable to allocate net_device " "structure!\n");
      break;
    }
    ...
    if (!dev->priv)
    {
      printk ("acenic: Unable to allocate memory.\n");
      return -ENOMEM;
    }
    ap = dev->priv;
    ap->pdev = pdev;
    ...
    if (!ap->regs)
    {
      printk ("%s:  Unable to map I/O register, " "AceNIC %i will be disabled.\n", dev->name, boards_found);
      break;
    }
    ...
    switch (pdev->vendor)
    {
         ...
         case PCI_VENDOR_ID_DEC:
           ...
           {
             sprintf (ap->name, "Farallon PN9000-SX " "Gigabit Ethernet");
             printk (KERN_INFO "%s: Farallon PN9000-SX ", dev->name);
             break;
           }
    ...
    {
      printk ("%s: Driver compiled without Tigon I" " support - NIC disabled\n", dev->name);
      iounmap (ap->regs);
...
{
  DWORD idx = readl (&regs->CmdPrd);

  writel (*(DWORD*)cmd, &regs->CmdRng[idx]);
...
static int __init ace_init (struct net_device *dev, int board_idx)
{
  ...
  DWORD tmp_ptr, myjif;
  DWORD tig_ver, mac1, mac2, tmp, pci_state;
  short i;
  ...
  switch (tig_ver)
  {
       ...
       case 4:
         printk (KERN_INFO "  Tigon I (Rev. 4), Firmware: %i.%i.%i, ",
                 tigonFwReleaseMajor, tigonFwReleaseMinor, tigonFwReleaseFix);
       ...
       case 6:
         printk (KERN_INFO "  Tigon II (Rev. %i), Firmware: %i.%i.%i, ",
                 tig_ver, tigon2FwReleaseMajor, tigon2FwReleaseMinor, tigon2FwReleaseFix);
         ...
         break;
       default:
         printk (KERN_INFO "  Unsupported Tigon version detected (%i), ", tig_ver);
         return -ENODEV;
  }
   ...
   */
  writel (ACE_BYTE_SWAP_DATA | ACE_WARN|ACE_FATAL | ACE_WORD_SWAP | \
  ...
  printk ("MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
          (mac1 >> 8) & 0xff, mac1 & 0xff, (mac2 >> 24) & 0xff, (mac2 >> 16) & 0xff, (mac2 >> 8) & 0xff, mac2 & 0xff);
  ...
  if (ap->version >= 2)
  {
    ...
    if (board_idx > 7 || dis_pci_mem_inval[board_idx])
    {
      if (ap->pci_command & PCI_COMMAND_INVALIDATE)
      {
        ...
        printk (KERN_INFO "%s: disabling PCI memory " "write and invalidate\n", dev->name);
      }
    }
      ...
      switch (L1_CACHE_BYTES)
      {
           ...
           default:
             printk (KERN_INFO "  Cache line size %i not "
                     "supported, PCI write and invalidate " "disabled\n", L1_CACHE_BYTES);
             ap->pci_command &= ~PCI_COMMAND_INVALIDATE;
             pci_write_config_word (ap->pdev, PCI_COMMAND, ap->pci_command);
      }
    }
  ...
  memset (ap->rx_mini_ring, 0, RX_MINI_RING_ENTRIES * sizeof (struct rx_desc));
  ...
  if (ap->version >= 2)
  {
    ...
    for (i = 0; i < RX_MINI_RING_ENTRIES; i++)
      ap->rx_mini_ring[i].flags = BD_FLG_TCP_UDP_SUM | BD_FLG_MINI;
  }
  else
  {
  ...
  memset (ap->rx_return_ring, 0, RX_RETURN_RING_ENTRIES * sizeof (struct rx_desc));
  ...
  if (board_idx >= 8)
  {
    printk ("%s: more then 8 NICs detected, "
  ...
  if (board_idx >= 0)
  {
    if (tx_coal_tick[board_idx])
       writel (tx_coal_tick[board_idx], &regs->TuneTxCoalTicks);
    if (max_tx_desc[board_idx])
       writel (max_tx_desc[board_idx], &regs->TuneMaxTxDesc);

    if (rx_coal_tick[board_idx])
       writel (rx_coal_tick[board_idx], &regs->TuneRxCoalTicks);
  ...
  if ((board_idx >= 0) && link[board_idx])
  {
    ...
    if ((option & 0x70) == 0)
    {
      printk ("%s: No media speed specified, forcing auto negotiation\n",
              dev->name);
      tmp |= LNK_NEGOTIATE | LNK_1000MB | LNK_100MB | LNK_10MB;
    }
    if ((option & 0x100) == 0)
         tmp |= LNK_NEG_FCTL;
    else printk ("%s: Disabling flow control negotiation\n", dev->name);

    if (option & 0x200)
       tmp |= LNK_RX_FLOW_CTL_Y;
    if ((option & 0x400) && (ap->version >= 2))
    {
      printk ("%s: Enabling TX flow control\n", dev->name);
      tmp |= LNK_TX_FLOW_CTL_Y;
    }
  }
  ...
  else printk ("%s: Someone is busy refilling the RX ring\n", dev->name);

  if (ap->version >= 2)
  {
    if (!test_and_set_bit (0, &ap->mini_refill_busy))
         ace_load_mini_rx_ring (ap, RX_MINI_SIZE);
    else printk ("%s: Someone is busy refilling the RX mini ring\n", dev->name);
  }
  return 0;
}
...
static void ace_timer (DWORD data)
{
  ...
  if (ap->tx_csm != ap->tx_ret_csm)
     printk ("%s: Transmitter is stuck, %08x\n",
...
static int ace_flush_jumbo_rx_ring (struct net_device *dev)
{
  ...
  if (ap->jumbo)
  {
    ...
    printk ("%s: Trying to flush Jumbo ring without Jumbo support enabled\n",
            dev->name);
 ...
 */
static DWORD ace_handle_event (struct net_device *dev, DWORD evtcsm, DWORD evtprd)
{
  struct ace_private *ap = (struct ace_private *) dev->priv;
      ...
      case E_ERROR:
           switch (ap->evt_ring[evtcsm].code)
           {
             case E_C_ERR_INVAL_CMD:
                  printk ("%s: invalid command error\n", dev->name);
                  break;
             case E_C_ERR_UNIMP_CMD:
                  printk ("%s: unimplemented command " "error\n", dev->name);
                  break;
             case E_C_ERR_BAD_CFG:
                  printk ("%s: bad config error\n", dev->name);
                  break;
             default:
                  printk ("%s: unknown error %02x\n",
...
static void ace_rx_int (struct net_device *dev, DWORD rxretprd, DWORD rxretcsm)
{
  struct ace_private *ap = (struct ace_private *) dev->priv;
  DWORD  idx = rxretcsm;
  ...
  while (idx != rxretprd)
  {
    ...
    desc_type = ap->rx_return_ring[idx].flags & (BD_FLG_JUMBO | BD_FLG_MINI);
    ...
    switch (desc_type)
    {
         ...
         default:
           printk (KERN_INFO "%s: unknown frame type (0x%02x) "
                   "returned by NIC\n", dev->name, ap->rx_return_ring[idx].flags);
           goto error;
    }
    ...
    *oldskb_p = NULL;
     ...
     */
    if (!(csum ^ 0xffff) && (!(((struct iphdr *) skb->data)->frag_off & __constant_htons (IP_MF | IP_OFFSET))))
...
static void ace_interrupt (int irq, void *dev_id, struct pt_regs *ptregs)
{
  ...
  if (dev->start)
  {
    ...
    if (cur_size < RX_LOW_STD_THRES)
    {
      if ((cur_size < RX_PANIC_STD_THRES) && !test_and_set_bit (0, &ap->std_refill_busy))
      {
#if DEBUG
    ...
    if (!ACE_IS_TIGON_I (ap))
    {
      ...
      if (cur_size < RX_LOW_MINI_THRES)
      {
        if ((cur_size < RX_PANIC_MINI_THRES) && !test_and_set_bit (0, &ap->mini_refill_busy))
        {
#if DEBUG
    ...
    if (ap->jumbo)
    {
      ...
      {
        if ((cur_size < RX_PANIC_JUMBO_THRES) && !test_and_set_bit (0, &ap->jumbo_refill_busy))
        {
#if DEBUG
...
static int ace_open (struct net_device *dev)
{
  ...
  if (ap->jumbo && !test_and_set_bit (0, &ap->jumbo_refill_busy))
...
static int ace_start_xmit (struct sk_buff *skb, struct net_device *dev)
{
  ...
  if ((idx + 1) % TX_RING_ENTRIES == ap->tx_ret_csm)
  {
    ap->tx_full = 1;
#if DEBUG
    printk ("%s: trying to transmit while the tx ring is full " "- this should not happen!\n", dev->name);
#endif
    return 1;
  }
...
static int ace_change_mtu (struct net_device *dev, int new_mtu)
{
  ...
  if (new_mtu > ACE_STD_MTU)
  {
    if (!(ap->jumbo))
    {
      printk (KERN_INFO "%s: Enabling Jumbo frame " "support\n", dev->name);
      ap->jumbo = 1;
...
static int ace_ioctl (struct net_device *dev, struct ifreq *ifr, int cmd)
{
  ...
  if (ecmd.cmd == ETH_GSET)
  {
    ecmd.supported =
      (SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full |
       ...
       SUPPORTED_1000baseT_Half | SUPPORTED_1000baseT_Full | SUPPORTED_Autoneg | SUPPORTED_FIBRE);
  ...
  else if (ecmd.cmd == ETH_SSET)
  {
    ...
    link = LNK_ENABLE | LNK_1000MB | LNK_100MB | LNK_10MB | LNK_RX_FLOW_CTL_Y | LNK_NEG_FCTL;
    if (!ACE_IS_TIGON_I (ap))
...
void __init ace_copy (struct ace_regs *regs, void *src, DWORD dest, int size)
{
  DWORD tdest;
  DWORD *wsrc;
  short tsize, i;
  ...
  while (size > 0)
  {
    tsize = min (((~dest & (ACE_WINDOW_SIZE - 1)) + 1), min (size, ACE_WINDOW_SIZE));
    tdest = (DWORD) & regs->Window + (dest & (ACE_WINDOW_SIZE - 1));
...
void __init ace_clear (struct ace_regs *regs, DWORD dest, int size)
{
  DWORD tdest;
  short tsize = 0, i;
...
int __init ace_load_firmware (struct net_device *dev)
{
  ...
  {
    printk ("%s: trying to download firmware while the " "CPU is running!\n", dev->name);
    return -EFAULT;
  }
  ...
  if (ACE_IS_TIGON_I (ap))
  {
    ...
    ace_copy (regs, tigonFwRodata, tigonFwRodataAddr, tigonFwRodataLen);
...
{
  DWORD local = readl (&regs->LocalCtrl);
 ...
 */
static u8 __init read_eeprom_byte (struct ace_regs *regs, DWORD offset)
{
  DWORD local;
  short i;
  u8 result = 0;
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    result = (result << 1) | ((readl (&regs->LocalCtrl) & EEPROM_DATA_IN) != 0);
    udelay (1);
    mb ();
    local = readl (&regs->LocalCtrl);
    local &= ~EEPROM_CLK_OUT;
