...
struct scp_struct
{
  WORD  zero_dum0;  /* has to be zero */
  BYTE  sysbus;     /* 0=16Bit,1=8Bit */
  BYTE  zero_dum1;  /* has to be zero for 586 */
  WORD  zero_dum2;
  WORD  zero_dum3;
...
struct iscp_struct
{
  BYTE  busy;          /* 586 clears after successful init */
  BYTE  zero_dummy;    /* has to be zero */
  WORD  scb_offset;    /* pointeroffset to the scb_base */
...
struct scb_struct
{
  BYTE rus;
  BYTE cus;
  BYTE cmd_ruc;        /* command word: RU part */
  BYTE cmd_cuc;        /* command word: CU part & ACK */
  WORD cbl_offset;     /* pointeroffset, command block list */
  WORD rfa_offset;     /* pointeroffset, receive frame area */
  WORD crc_errs;       /* CRC-Error counter */
  WORD aln_errs;       /* allignmenterror counter */
  WORD rsc_errs;       /* Resourceerror counter */
  WORD ovrn_errs;      /* OVerrunerror counter */
};
...
struct rfd_struct
{
  BYTE  stat_low;           /* status word */
  BYTE  stat_high;          /* status word */
  BYTE  rfd_sf;             /* 82596 mode only */
  BYTE  last;               /* Bit15,Last Frame on List / Bit14,suspend */
  WORD  next;               /* linkoffset to next RFD */
  WORD  rbd_offset;         /* pointeroffset to RBD-buffer */
  BYTE  dest[6];            /* ethernet-address, destination */
  BYTE  source[6];          /* ethernet-address, source */
  WORD  length;             /* 802.3 frame-length */
  WORD  zero_dummy;         /* dummy */
};
...
struct rbd_struct 
{
  WORD  status;       /* status word,number of used bytes in buff */
  WORD  next;         /* pointeroffset to next RBD */
  char *buffer;       /* receive buffer address pointer */
  WORD  size;         /* size of this buffer */
  WORD  zero_dummy;   /* dummy */
};
...
{
  WORD cmd_status;    /* status of this command */
  WORD cmd_cmd;       /* the command itself (+bits) */
  WORD cmd_link;      /* offsetpointer to next command */
};
...
struct configure_cmd_struct
{
  ...
  WORD  cmd_cmd;
  WORD  cmd_link;
  BYTE  byte_cnt;    /* size of the config-cmd */
  BYTE  fifo;        /* fifo/recv monitor */
  BYTE  sav_bf;      /* save bad frames (bit7=1)*/
  BYTE  adr_len;     /* adr_len(0-2),al_loc(3),pream(4-5),loopbak(6-7)*/
  BYTE  priority;    /* lin_prio(0-2),exp_prio(4-6),bof_metd(7) */
  BYTE  ifs;         /* inter frame spacing */
  BYTE  time_low;    /* slot time low */
  BYTE  time_high;   /* slot time high(0-2) and max. retries(4-7) */
  BYTE  promisc;     /* promisc-mode(0) , et al (1-7) */
  BYTE  carr_coll;   /* carrier(0-3)/collision(4-7) stuff */
  BYTE  fram_len;    /* minimal frame len */
  BYTE  dummy;       /* dummy */
};
...
struct mcsetup_cmd_struct 
{
  ...
  WORD  cmd_cmd;
  WORD  cmd_link;
  WORD  mc_cnt;           /* number of bytes in the MC-List */
  BYTE  mc_list[0][6];    /* pointer to 6 bytes entries */
};
...
struct dump_cmd_struct
{
  ...
  WORD  cmd_cmd;
  WORD  cmd_link;
  WORD  dump_offset;    /* pointeroffset to DUMP space */
};
...
struct transmit_cmd_struct 
{
  ...
  WORD  cmd_cmd;
  WORD  cmd_link;
  WORD  tbd_offset;    /* pointeroffset to TBD */
  BYTE  dest[6];       /* destination address of the frame */
  WORD  length;        /* user defined: 802.3 length / Ether type */
};
...
struct tbd_struct
{
  WORD  size;           /* size + EOF-Flag(15) */
  WORD  next;           /* pointeroffset to next TBD */
