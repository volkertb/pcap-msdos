...
static DWORD lance_portlist[] = {0x300, 0x320, 0x340, 0x360, 0};
...
#define LANCE_KMALLOC(x) \
  ((void *) (((DWORD)kmalloc((x)+7, GFP_DMA | GFP_KERNEL)+7) & ~7))
...
struct lance_private {
  ...
  DWORD rx_buffs;    /* Address of Rx and Tx buffers. */
  ...
  BYTE chip_version;  /* See lance_chip_type. */
  char tx_full;
  DWORD lock;
};
...
static BYTE lance_need_isa_bounce_buffers = 1;
...
void lance_probe1(int ioaddr)
{
  ...
  BYTE hpJ2405A = 0;      /* HP ISA adaptor */
  ...
  lp = (struct lance_private *)(((DWORD)kmalloc(sizeof(*lp)+7,
                       GFP_DMA | GFP_KERNEL)+7) & ~7);
  if (lance_debug > 6) printk(" (#0x%05lx)", (DWORD)lp);
  memset(lp, 0, sizeof(*lp));
  dev->priv = lp;
  lp->name = chipname;
  lp->rx_buffs = (DWORD)kmalloc(PKT_BUF_SZ*RX_RING_SIZE,
                      GFP_DMA | GFP_KERNEL);
...
static void
lance_restart(struct device *dev, DWORD csr0_bits, int must_reinit)
{
  struct lance_private *lp = (struct lance_private *)dev->priv;
    ...
    else 
    {
      ...
      else
      {
        ...
        eth_copy_and_sum(skb,
          (BYTE *)bus_to_virt((lp->rx_ring[entry].base & 0x00ffffff)),
          pkt_len,0);
