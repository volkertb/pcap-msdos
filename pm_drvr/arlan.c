...
extern inline uint64 arlan_time(void)
{
	struct timeval timev;
	do_gettimeofday(&timev);
        return ((uint64) timev.tv_sec * 1000000 + timev.tv_usec);
};
...
static struct enet_statistics *arlan_statistics(struct net_device *dev)
{
	...
	/* Update the statistics from the device registers. */
        ...
        READSHM(priv->stats.tx_collisions, arlan->numReTransmissions, u_int);
