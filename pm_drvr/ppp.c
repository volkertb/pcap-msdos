...
static int ppp_tty_write (struct tty_struct *, struct file *, const __u8 *,
        DWORD);
static int ppp_tty_ioctl (struct tty_struct *, struct file *, DWORD,
                          DWORD);
static DWORD ppp_tty_poll (struct tty_struct *tty, struct file *filp, poll_table * wait);
...
#define in_rmap(ppp,c)  ((((DWORD) (__u8) (c)) < 0x20) && \
 ...
 */
  ...
  dev->mem_start  = (DWORD) buf_base (new_wbuf);
  dev->mem_end  = (DWORD) (dev->mem_start + mtu);
  dev->rmem_start = (DWORD) buf_base (new_rbuf);
  dev->rmem_end  = (DWORD) (dev->rmem_start + mru);
/*
...
static void ppp_dev_xmit_lower (struct ppp *ppp, struct ppp_buffer *buf,
 ...
 */
  ...
  cp = find_compressor ((int) (DWORD) (__u8) ccp_option[0]);
