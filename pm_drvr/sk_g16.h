...
struct init_block 
{
  ...
  BYTE  paddr[6]; /* Physical Address (MAC) */
  BYTE  laddr[8]; /* Logical Filter Address (not used) */
  DWORD   rdrp;     /* Receive Descriptor Ring pointer */
  DWORD   tdrp;     /* Transmit Descriptor Ring pointer */
};
...
struct tmd   
{
  ...
  WORD blen;             /* Buffer Length (two's complement) */
