...
struct tulip_private {
  ...
  DWORD tx_full:1;        /* The Tx queue is full. */
  DWORD full_duplex:1;      /* Full-duplex operation requested. */
  DWORD port_fix:1;      /* Fix if_port to specified port. */
};
...
#endif
    for (i = 0; i < RX_RING_SIZE; i++)
      printk(" %8.8x", (DWORD)tp->rx_ring[i].status);
#ifndef  __alpha__
    printk("\n  Tx ring %8.8x: ", (int)tp->tx_ring);
#endif
    for (i = 0; i < TX_RING_SIZE; i++)
      printk(" %8.8x", (DWORD)tp->tx_ring[i].status);
    printk("\n");
...
#ifdef ETHER_STATS
          if (status & 0x0100) l
             p->stats.collisions16++;
#endif
        } else {
#ifdef ETHER_STATS
          if (status & 0x0001) lp->stats.tx_deferred++;
#endif
          lp->stats.tx_collisions += (status >> 3) & 15;
          lp->stats.tx_packets++;
        }
       ...
       int irq, int device_id)
{
  ...
  static BYTE last_phys_addr[6] = {0x00, 'L', 'i', 'n', 'u', 'x'};
  static int last_irq;
