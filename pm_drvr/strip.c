...
typedef struct
{
    ...
    WORD  protocol;    /* The protocol type, using Ethernet codes */
} STRIP_Header;
...
struct strip
{
    ...
    BYTE     *rx_buff;      /* buffer for received IP packet*/
    BYTE     *sx_buff;      /* buffer for received serial data*/
    int                sx_count;    /* received serial data counter */
    int                sx_size;      /* Serial buffer size    */
    BYTE     *tx_buff;      /* transmitter buffer           */
    BYTE     *tx_head;      /* pointer to next byte to XMIT */
    ...
    DWORD      rx_packets;    /* inbound frames counter  */
    DWORD      tx_packets;    /* outbound frames counter  */
    DWORD      rx_errors;    /* Parity, etc. errors    */
    DWORD      tx_errors;    /* Planned stuff    */
    DWORD      rx_dropped;    /* No memory for skb    */
    DWORD      tx_dropped;    /* When MTU change    */
    DWORD      rx_over_errors;    /* Frame bigger then STRIP buf. */
 ...
 */
static const WORD  MAX_STRIP_MTU          = 1132;
static const WORD  DEFAULT_STRIP_MTU      = 1024;
...
static void strip_changedmtu(struct strip *strip_info)
{
    ...
    struct device *dev    = &strip_info->dev;
    BYTE *orbuff = strip_info->rx_buff;
    BYTE *osbuff = strip_info->sx_buff;
    BYTE *otbuff = strip_info->tx_buff;
...
static int slip_len(BYTE *data, int len)
{
    static const BYTE SLIP_END=0300;  /* indicates end of SLIP frame  */
    static const BYTE SLIP_ESC=0333;  /* indicates SLIP byte stuffing  */
    int count = len;
    while (--len >= 0)
    {
...
static void jiffiestotimeval(DWORD jiffies, struct timeval *value)
{
...
static BYTE *strip_make_packet(BYTE *ptr, struct strip *strip_info, struct sk_buff *skb)
{
#if DO_PROC_NET_STRIP_TRACE
    BYTE *start_ptr;
#endif DO_PROC_NET_STRIP_TRACE
...
static void strip_IdleTask(DWORD parameter)
{
...
static void
strip_receive_buf(struct tty_struct *tty, const BYTE *cp, char *fp, int count)
{
    struct strip *strip_info = (struct strip *) tty->disc_data;
    const BYTE *end = cp + count;
...
static int strip_proc_net_register(WORD type, char *file_name,
