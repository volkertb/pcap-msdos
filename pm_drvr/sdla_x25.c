...
typedef struct x25_channel
{
  ...
  WORD protocol;  /* ethertype, 0 - multiplexed */
...
static BYTE bps_to_speed_code (DWORD bps);
static DWORD dec_to_uint (BYTE* str, int len);
static DWORD hex_to_uint (BYTE* str, int len);
static void parse_call_info (BYTE* str, x25_call_info_t* info);
...
static int if_init (struct device* dev)
{
  ...
  if (!chan->svc)
    *(WORD*)dev->dev_addr = htons(chan->lcn)
  ;
