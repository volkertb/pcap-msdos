...
static struct device *dfx_alloc_device (struct device *dev, WORD iobase);
...
static void  dfx_close (struct device *dev);  
static void  dfx_int_pr_halt_id (DFX_board_t *bp);
static void  dfx_int_type_0_process (DFX_board_t *bp);
static void  dfx_int_common (DFX_board_t *bp);
static void  dfx_interrupt (int irq);
static void *dfx_ctl_get_stats (struct device *dev);
...
static int   dfx_hw_port_ctrl_req (DFX_board_t *bp, PI_UINT32 command,
                                   PI_UINT32 data_a, PI_UINT32 data_b,
                                   PI_UINT32 *host_data);
 ...
 */
static inline void dfx_port_write_byte (DFX_board_t *bp, int offset, BYTE data)
{
  WORD port = bp->base_addr + offset;
  outb (data, port);
}

static inline void dfx_port_read_byte (DFX_board_t *bp, int offset, BYTE *data)
{
  WORD port = bp->base_addr + offset;
  *data = inb (port);
}

static inline void dfx_port_write_long (DFX_board_t *bp, int offset, DWORD data)
{
  WORD port = bp->base_addr + offset;
  outl (data, port);
}

static inline void dfx_port_read_long (DFX_board_t *bp, int offset, DWORD *data)
{
  WORD port = bp->base_addr + offset;
  *data = inl (port);
}
...
int dfx_probe (struct device *dev)
{
  ...
  BYTE  pci_bus;             /* PCI bus number (0-255) */
  BYTE  pci_dev_fun;         /* PCI device and function numbers (0-255) */
  WORD  port;                /* temporary I/O (port) address */
  WORD  command;             /* PCI Configuration space Command register val */
  DWORD slot_id;             /* EISA hardware (slot) ID read from adapter */
  ...
  if (already_probed)
  {
    ...
    if (dev && !strncmp(dev->name,"fddi",4) && dev->base_addr)
       DBG_printk ("In dfx_probe for fddi adapter (%s) we've already "
                   "initialized it, so return success\n", dev->name);
    return (1);
  }
  ...
  for (i = 0; i < DFX_MAX_EISA_SLOTS; i++) /* only scan for up to 16 EISA slots */
  {
	...
	printk (version);	/* we only display this string ONCE */
      ...
      if (dev)
      {
        ...
        else dev->base_addr = 0; /* clear port address field in device structure on failure */
      }
    }
  }
  ...
  if (pcibios_present ())	/* is PCI BIOS even present? */
    for (i = 0; i < DFX_MAX_NUM_BOARDS; i++)	/* scan for up to 8 PCI cards */
      if (pcibios_find_device(PCI_VENDOR_ID_DEC, PCI_DEVICE_ID_DEC_FDDI,
 ...
 *   None
 */
struct device *dfx_alloc_device (struct device *dev, WORD iobase)
{
   ...
   */
  for (tmp_dev = dev; tmp_dev; tmp_dev = tmp_dev->next)
    if (!strncmp (tmp_dev->name, "fddi", 4) && tmp_dev->base_addr == 0)
       break;
   ...
   */
  tmp_dev->priv = k_calloc (sizeof(DFX_board_t), 1);
  if (!tmp_dev->priv)
  {
  ...
  tmp_dev->close     = dfx_close;
  tmp_dev->xmit      = dfx_xmt_queue_pkt;
 ...
 *   enabled yet.
 */
void dfx_bus_init (struct device *dev)
{
  DFX_board_t *bp = (DFX_board_t *) dev->priv;
  BYTE         val;                /* used for I/O read/writes */
  ...
  if (bp->bus_type == DFX_BUS_TYPE_EISA)
  {
    ...
    /* Enable access to I/O on the board by writing 0x03 to
     * Function Control Register
     */
    dfx_port_write_byte (bp, PI_ESIC_K_FUNCTION_CNTRL,
    ...
    dfx_port_write_byte (bp, PI_ESIC_K_BURST_HOLDOFF,
                         (val & ~PI_BURST_HOLDOFF_M_MEM_MAP));

    /* Enable interrupts at EISA bus interface chip (ESIC)
     */
    dfx_port_read_byte (bp, PI_ESIC_K_IO_CONFIG_STAT_0, &val);
    dfx_port_write_byte (bp, PI_ESIC_K_IO_CONFIG_STAT_0,
  ...
  else
  {
    ...
    if (val < PFI_K_LAT_TIMER_MIN)	/* if less than min, override with default */
    {
      ...
      pcibios_write_config_byte (bp->pci_bus, bp->pci_dev_fun,
...
void dfx_bus_config_check (DFX_board_t *bp)
{
  int   status;            /* return code from adapter port control call */
  DWORD slot_id;           /* EISA-bus hardware id (DEC3001, DEC3002,...) */
  DWORD host_data;         /* LW data returned from port control call */
  ...
  if (bp->bus_type == DFX_BUS_TYPE_EISA)
  {
    ...
    if (slot_id == DEFEA_PROD_ID_2)
    {
      ...
      status = dfx_hw_port_ctrl_req (bp, PI_PCTRL_M_SUB_CMD,
                                     PI_SUB_CMD_K_PDQ_REV_GET, 0, &host_data);
 ...
 */
int dfx_driver_init (struct device *dev)
{
  DFX_board_t *bp = (DFX_board_t*) dev->priv;
  int   alloc_size;               /* total buffer size needed */
  char *top_v, *curr_v;           /* virtual addrs into memory block */
  DWORD top_p, curr_p;            /* physical addrs into memory block */
  DWORD data;                     /* host data register value */
   ...
   */
  dfx_hw_dma_uninit (bp, PI_PDATA_A_RESET_M_SKIP_ST);
  ...
  memcpy (&bp->factory_mac_addr[0], &data, sizeof (DWORD));

  if (dfx_hw_port_ctrl_req (bp, PI_PCTRL_M_MLA, PI_PDATA_A_MLA_K_HI, 0, &data) != DFX_K_SUCCESS)
  {
    printk ("%s: Could not read adapter factory MAC address!\n", dev->name);
    return (DFX_K_FAILURE);
  }
  memcpy (&bp->factory_mac_addr[4], &data, sizeof (WORD));
  ...
  if (bp->bus_type == DFX_BUS_TYPE_EISA)
       printk ("%s: DEFEA at I/O addr = 0x%lX, IRQ = %d, Hardware addr = "
               "%02X-%02X-%02X-%02X-%02X-%02X\n",
               dev->name, dev->base_addr, dev->irq,
               dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2],
               dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]);
  else printk ("%s: DEFPA at I/O addr = 0x%lX, IRQ = %d, Hardware addr = "
               "%02X-%02X-%02X-%02X-%02X-%02X\n",
               dev->name, dev->base_addr, dev->irq,
               dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2],
               dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]);
  ...
  curr_p = (DWORD) (ALIGN (top_p, PI_ALIGN_K_DESC_BLK));
 ...
 */
int dfx_adap_init (DFX_board_t *bp)
{
  ...
  dfx_port_write_long (bp, PI_PDQ_K_REG_TYPE_0_STATUS,
                           PI_HOST_INT_K_ACK_ALL_TYPE_0);

  /* Clear Type 1 and Type 2 registers before going to DMA_AVAILABLE state
   *
   * Note: We only need to clear host copies of these registers.  The PDQ
   *       reset takes care of the on-board register values.
   */
   ...
   */
  if (dfx_hw_port_ctrl_req (bp, PI_PCTRL_M_SUB_CMD, PI_SUB_CMD_K_BURST_SIZE_SET,
                            bp->burst_size, NULL) != DFX_K_SUCCESS)
  {
  ...
  if (dfx_hw_port_ctrl_req (bp, PI_PCTRL_M_CONS_BLOCK, bp->cons_block_phys,
  ...
  if (dfx_hw_port_ctrl_req (bp,
			    PI_PCTRL_M_INIT,
                            (DWORD) (bp->descr_block_phys |
                            PI_PDATA_A_INIT_M_BSWAP_DATA), 0, NULL) != DFX_K_SUCCESS)
  {
 ...
 */
int dfx_open (struct device *dev)
{
  DFX_board_t *bp = (DFX_board_t *) dev->priv;

  DBG_printk ("In dfx_open...\n");

  if (!request_irq (dev->irq, dfx_interrupt))
  {
  ...
  dev->tx_busy = 0;
  dev->reentry = DFX_UNMASK_INTERRUPTS;
  dev->start   = 1;
  return (1);
}
 ...
 *   routine.
 */
void dfx_close (struct device *dev)
{
  DFX_board_t *bp = (DFX_board_t *) dev->priv;
...
void dfx_int_pr_halt_id (DFX_board_t *bp)
{
 ...
 */
void dfx_int_type_0_process (DFX_board_t *bp)
{
   ...
   */
  if (type_0_status & (PI_TYPE_0_STAT_M_NXM | PI_TYPE_0_STAT_M_PM_PAR_ERR | PI_TYPE_0_STAT_M_BUS_PAR_ERR))
  {
  ...
  if (type_0_status & PI_TYPE_0_STAT_M_XMT_FLUSH)
  {
    ...
    dfx_hw_port_ctrl_req (bp, PI_PCTRL_M_XMT_DATA_FLUSH_DONE, 0, 0, NULL);
  }
 ...
 */
void dfx_int_common (DFX_board_t *bp)
{
  ...
  /* Read PDQ Port Status register to find out which interrupts
   * need processing
   */
  dfx_port_read_long (bp, PI_PDQ_K_REG_PORT_STATUS, &port_status);

  /* Process Type 0 interrupts (if any) - infrequent, so only call when
   * needed
   */
 ...
 */
void dfx_interrupt (int irq)
{
  struct device *dev = irq2dev_map [irq];
  DFX_board_t   *bp;     /* private board structure pointer */
  BYTE           tmp;    /* used for disabling/enabling ints */
  ...
  dev->reentry = DFX_MASK_INTERRUPTS; /* ensure non reentrancy */
  ...
  if (bp->bus_type == DFX_BUS_TYPE_PCI)
  {
    ...
    dfx_port_write_long (bp, PFI_K_REG_MODE_CTRL, PFI_MODE_M_PDQ_INT_ENB +
                                                  PFI_MODE_M_DMA_ENB);
  }
  else
  {
 ...
 *   None
 */
void *dfx_ctl_get_stats (struct device *dev)
{
  DFX_board_t *bp = (DFX_board_t *) dev->priv;
  ...
  bp->stats.rx_errors  = (DWORD) (bp->rcv_crc_errors + bp->rcv_frame_status_errors + bp->rcv_length_errors);
 ...
 */
void dfx_ctl_set_multicast_list (struct device *dev)
{
  DFX_board_t        *bp = (DFX_board_t *) dev->priv;
  ...
  else   /* Else, update multicast address table */
  {
    ...
    else DBG_printk ("%s: Multicast address table updated!  Added %d addresses.\n",
  ...
  else DBG_printk ("%s: Adapter filters updated!\n", dev->name);
}
 ...
 */
#if 0
int dfx_ctl_set_mac_address (struct device *dev, void *addr)
{
  DFX_board_t     *bp         = (DFX_board_t *) dev->priv;
  ...
  else DBG_printk ("%s: Adapter CAM updated with new MAC address\n", dev->name);
  return (1);                   /* always return zero */
}
#endif
 ...
 */
int dfx_ctl_update_cam (DFX_board_t *bp)
{
  PI_LAN_ADDR *p_addr;
  int i;
 ...
 */
int dfx_ctl_update_filters (DFX_board_t *bp)
{
  int i = 0;
 ...
 *
 * Functional Description:
 *   The command request and response buffers are posted to the adapter in
 *   the manner described in the PDQ Port Specification:
 *
 *    1. Command Response Buffer is posted to adapter.
 *    2. Command Request Buffer is posted to adapter.
 *    3. Command Request consumer index is polled until it indicates that
 *       request buffer has been DMA'd to adapter.
 *    4. Command Response consumer index is polled until it indicates that
 *       response buffer has been DMA'd from adapter.
 *
 ...
 *   None
 */
int dfx_hw_dma_cmd_req (DFX_board_t *bp)
{
   ...
   */
  bp->descr_block_virt->cmd_rsp[bp->cmd_rsp_reg.index.prod].long_0 =
    (DWORD) (PI_RCV_DESCR_M_SOP | ((PI_CMD_RSP_K_SIZE_MAX /
             PI_ALIGN_K_CMD_RSP_BUFF) << PI_RCV_DESCR_V_SEG_LEN));

  bp->descr_block_virt->cmd_rsp[bp->cmd_rsp_reg.index.prod].long_1 =
    bp->cmd_rsp_phys;
   ...
   */
  bp->descr_block_virt->cmd_req[bp->cmd_req_reg.index.prod].long_0 =
    (DWORD) (PI_XMT_DESCR_M_SOP | PI_XMT_DESCR_M_EOP |
             (PI_CMD_REQ_K_SIZE_MAX << PI_XMT_DESCR_V_SEG_LEN));

  bp->descr_block_virt->cmd_req[bp->cmd_req_reg.index.prod].long_1 =
    bp->cmd_req_phys;
  ...
  {
    if (bp->cmd_req_reg.index.prod == (BYTE) (bp->cons_block_virt->cmd_req))
       break;
  ...
  for (timeout_cnt = 20000; timeout_cnt > 0; timeout_cnt--)
  {
    if (bp->cmd_rsp_reg.index.prod == (BYTE) (bp->cons_block_virt->cmd_rsp))
       break;
    udelay (100);		/* wait for 100 microseconds */
  }
  if (timeout_cnt == 0)
 ...
 *   None
 */
int dfx_hw_port_ctrl_req (DFX_board_t *bp,   PI_UINT32 command,
                          PI_UINT32  data_a, PI_UINT32 data_b,
                          PI_UINT32 *host_data)
{
  ...
  else timeout_cnt = 20000;    /* set command timeout count to 2 seconds */
 ...
 */
void dfx_hw_adap_reset (DFX_board_t *bp, PI_UINT32 type)
{
  ...
  /* Wait for at least 1 Microsecond according to the spec.
   * We wait 20 just to be safe
   */
  udelay (20);
 ...
 *   None
 */
int dfx_hw_adap_state_rd (DFX_board_t *bp)
{
 ...
 *   Internal adapter registers are cleared.
 */
int dfx_hw_dma_uninit (DFX_board_t *bp, PI_UINT32 type)
{
  int timeout_cnt;		/* used in for loops */
 ...
 *   is notified.
 */
void dfx_rcv_init (DFX_board_t *bp)
{
  int i, j;
  ...
  for (i = 0; i < (int) (bp->rcv_bufs_to_post); i++)
    for (j = 0; (i + j) < (int)PI_RCV_DATA_K_NUM_ENTRIES; j += bp->rcv_bufs_to_post)
    {
      bp->descr_block_virt->rcv_data[i + j].long_0 =
        (DWORD) (PI_RCV_DESCR_M_SOP | ((PI_RCV_DATA_K_SIZE_MAX /
                 PI_ALIGN_K_RCV_DATA_BUFF) << PI_RCV_DESCR_V_SEG_LEN));

      bp->descr_block_virt->rcv_data[i + j].long_1 =
        (DWORD) (bp->rcv_block_phys + (i * PI_RCV_DATA_K_SIZE_MAX));
      bp->p_rcv_buff_va[i + j] = (char*) (bp->rcv_block_virt +
                                          (i * PI_RCV_DATA_K_SIZE_MAX));
    }
 ...
 *   None
 */
void dfx_rcv_queue_process (DFX_board_t *bp)
{
  ...
  while (bp->rcv_xmt_reg.index.rcv_comp != p_type_2_cons->index.rcv_cons)
  {
    /* Process any errors
     */
    char *p_buff = (char*)bp->p_rcv_buff_va[bp->rcv_xmt_reg.index.rcv_comp];
    DWORD descr = *(DWORD*) (p_buff + RCV_BUFF_K_DESCR); /* FMC descr. field */
    int   len;
    ...
    if (descr & PI_FMC_DESCR_M_RCC_FLUSH)
    {
      if (descr & PI_FMC_DESCR_M_RCC_CRC)
           bp->rcv_crc_errors++;
      else bp->rcv_frame_status_errors++;
    }
    else
    {
      /* The frame was received without errors - verify packet length
       */
      len = (DWORD) ((descr & PI_FMC_DESCR_M_LEN) >> PI_FMC_DESCR_V_LEN);
      len -= 4;             /* subtract 4 byte CRC */
      if (!IN_RANGE (len, FDDI_K_LLC_ZLEN, FDDI_K_LLC_LEN))
      ...
      else
      {
        if (dev->get_rx_buf)
        {
          ...
          if (buf)
               memcpy (buf, p_buff + RCV_BUFF_K_PADDING, len + 3);
          else bp->rcv_discards++;
        }
 ...
 *   None
 */
int dfx_xmt_queue_pkt (struct device *dev, void *buf, int len)
{
  DFX_board_t      *bp = (DFX_board_t *) dev->priv;
  BYTE              prod;            /* local transmit producer index */
  ...
  *((char *) skb->data - 1) = DFX_PRH2_BYTE;	/* specification */
   ...
   *      wide.
   */
  p_xmt_descr->long_0 = (DWORD) (PI_XMT_DESCR_M_SOP | PI_XMT_DESCR_M_EOP | ((skb->len + 3) << PI_XMT_DESCR_V_SEG_LEN));
  p_xmt_descr->long_1 = (DWORD) virt_to_bus (skb->data - 3);
  ...
  if (prod == bp->rcv_xmt_reg.index.xmt_comp)
     return (0);                /* requeue packet for later */
  ...
  dfx_port_write_long (bp, PI_PDQ_K_REG_TYPE_2_PROD, bp->rcv_xmt_reg.lword);
  return (1);                   /* packet queued to adapter */
}
 ...
 *   None
 */
void dfx_xmt_done (DFX_board_t *bp)
{
  XMT_DRIVER_DESCR   *p_xmt_drv_descr; /* ptr to transmit driver descriptor */
  PI_TYPE_2_CONSUMER *p_type_2_cons;   /* ptr to rcv/xmt consumer block register */
 ...
 *   None
 */
void dfx_xmt_flush (DFX_board_t *bp)
{
  DWORD             prod_cons;       /* rcv/xmt consumer block longword */
  XMT_DRIVER_DESCR *p_xmt_drv_descr; /* ptr to transmit driver descriptor */
   ...
   */
  prod_cons = (DWORD) (bp->cons_block_virt->xmt_rcv_data & ~PI_CONS_M_XMT_INDEX);
  prod_cons |= (DWORD) (bp->rcv_xmt_reg.index.xmt_prod << PI_CONS_V_XMT_INDEX);
