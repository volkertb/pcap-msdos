...
struct e8390_pkt_hdr {
       ...
       WORD  count;    /* header + packet length in bytes */
     };
...
extern void ei_interrupt (int irq)  LOCKED_FUNC;

struct ei_device {
  const char *name;
  void (*reset_8390)  (struct device *);
  void (*get_8390_hdr)(struct device *, struct e8390_pkt_hdr *, int);
  void (*block_output)(struct device *, int, const char*, int);
  void (*block_input) (struct device *, int, char*, int);
  ...
  BYTE     rx_start_page;
  BYTE     stop_page;
  BYTE     current_page;    /* Read pointer in buffer  */
  BYTE     interface_num;   /* Net port (AUI, 10bT.) to use. */
  BYTE     txqueue;         /* Tx Packet buffer queue length. */
  short    tx1, tx2;        /* Packet lengths for ping-pong tx. */
  short    lasttx;          /* Alpha version consistency check. */
  BYTE     reg0;            /* Register '0' in a WD8013 */
  BYTE     reg5;            /* Register '5' in a WD8013 */
  BYTE     saved_irq;       /* Original dev->irq value. */
