...
static const char *version = "es3210.c: Driver revision v0.03, 14/09/96\n";

#include "pmdrvr.h"
#include "8390.h"

static int  es_probe1 (struct device *dev, int ioaddr);
static int  es_open   (struct device *dev);
static void es_close  (struct device *dev);
...
static void es_block_input  (struct device *dev, int count, char *buf, int ring_offset);
static void es_block_output (struct device *dev, int count, char *buf, int start_page);
...
static BYTE lo_irq_map[] = { 3, 4, 5, 6, 7, 9, 10 };
static BYTE hi_irq_map[] = { 11, 12, 0, 14, 0, 0, 0, 15 };
...
int es_probe1 (struct device *dev, int ioaddr)
{
...
#if (ES_DEBUG & ES_D_PROBE)
  printk ("es3210.c: probe at %3x, ID %8x\n", ioaddr, inl (ioaddr + ES_ID_PORT));
  ...
  printk ("es3210.c: ES3210 rev. %ld at %3x, node", eisa_id >> 24, ioaddr);
  for (i = 0; i < ETH_ALEN; i++)
  ...
  if (dev->irq == 0)
  {
    BYTE hi_irq = inb (ioaddr + ES_CFG2) & 0x07;
    BYTE lo_irq = inb (ioaddr + ES_CFG1) & 0xfe;
...
#if (ES_DEBUG & ES_D_PROBE)
    printk ("es3210.c: hi_irq %d, lo_irq %d, dev->irq = %d\n",
  ...
  if (dev->mem_start == 0)
  {
    BYTE mem_enabled = inb (ioaddr + ES_CFG2) & 0xc0;
    BYTE mem_bits    = inb (ioaddr + ES_CFG3) & 0x07;
  ...
  dev->mem_end    = dev->rmem_end = dev->mem_start + (ES_STOP_PG - ES_START_PG) * 256;
...
static void es_reset_8390 (struct device *dev)
{
  ...
  if (ei_debug > 1)
     printk ("%s: resetting the ES3210...", dev->name);

  end = jiffies + 2 * HZ / 100;
  while ((signed) (end - jiffies) > 0)
     continue;
...
static void es_get_8390_hdr (struct device *dev, struct e8390_pkt_hdr *hdr, int ring_page)
{
  DWORD hdr_start = dev->mem_start + ((ring_page - ES_START_PG) << 8);
...
static void es_block_input (struct device *dev, int count, char *buf, int ring_offset)
{
  DWORD xfer_start = dev->mem_start + ring_offset - (ES_START_PG << 8);
...
static void es_block_output (struct device *dev, int count, char *buf, int start_page)
{
  DWORD shmem = dev->mem_start + ((start_page - ES_START_PG) << 8);
