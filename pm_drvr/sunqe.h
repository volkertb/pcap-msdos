...
struct qe_globreg {
  ...
  volatile DWORD psize;     /* Packet Size              */
  volatile DWORD msize;     /* Local-mem size (64K)     */
  volatile DWORD rsize;     /* Receive partition size   */
  volatile DWORD tsize;     /* Transmit partition size  */
};
...
struct qe_creg {
  volatile DWORD ctrl;       /* Control                   */
  volatile DWORD stat;       /* Status                    */
  volatile DWORD rxds;       /* RX descriptor ring ptr    */
  volatile DWORD txds;       /* TX descriptor ring ptr    */
  volatile DWORD rimask;     /* RX Interrupt Mask         */
  volatile DWORD timask;     /* TX Interrupt Mask         */
  volatile DWORD qmask;      /* QEC Error Interrupt Mask  */
  volatile DWORD mmask;      /* MACE Error Interrupt Mask */
  volatile DWORD rxwbufptr;  /* Local memory rx write ptr */
  volatile DWORD rxrbufptr;  /* Local memory rx read ptr  */
  volatile DWORD txwbufptr;  /* Local memory tx write ptr */
  volatile DWORD txrbufptr;  /* Local memory tx read ptr  */
  volatile DWORD ccnt;       /* Collision Counter         */
  volatile DWORD pipg;       /* Inter-Frame Gap           */
};
...
struct qe_mregs {
  volatile BYTE rxfifo;   /* Receive FIFO                   */
  volatile BYTE txfifo;   /* Transmit FIFO                  */
  volatile BYTE txfcntl;  /* Transmit Frame Control         */
  volatile BYTE txfstat;  /* Transmit Frame Status          */
  volatile BYTE txrcnt;   /* Transmit Retry Count           */
  volatile BYTE rxfcntl;  /* Receive Frame Control          */
  volatile BYTE rxfstat;  /* Receive Frame Status           */
  volatile BYTE ffcnt;    /* FIFO Frame Count               */
  volatile BYTE ireg;     /* Interrupt Register             */
  volatile BYTE imask;    /* Interrupt Mask                 */
  volatile BYTE poll;     /* POLL Register                  */
  volatile BYTE bconfig;  /* BIU Config                     */
  volatile BYTE fconfig;  /* FIFO Config                    */
  volatile BYTE mconfig;  /* MAC Config                     */
  volatile BYTE plsconfig;/* PLS Config                     */
  volatile BYTE phyconfig;/* PHY Config                     */
  volatile BYTE chipid1;  /* Chip-ID, low bits              */
  volatile BYTE chipid2;  /* Chip-ID, high bits             */
  volatile BYTE iaconfig; /* Internal Address Config        */
  volatile BYTE _unused0; /* Reserved...                    */
  volatile BYTE filter;   /* Logical Address Filter         */
  volatile BYTE ethaddr;  /* Our Ethernet Address           */
  volatile BYTE _unused1; /* Reserved...                    */
  volatile BYTE _unused2; /* Reserved...                    */
  volatile BYTE mpcnt;    /* Missed Packet Count            */
  volatile BYTE _unused3; /* Reserved...                    */
  volatile BYTE rpcnt;    /* Runt Packet Count              */
  volatile BYTE rccnt;    /* RX Collision Count             */
  volatile BYTE _unused4; /* Reserved...                    */
  volatile BYTE utest;    /* User Test                      */
  volatile BYTE rtest1;   /* Reserved Test 1                */
  volatile BYTE rtest2;   /* Reserved Test 2                */
};
...
extern __inline__ struct sk_buff *qe_alloc_skb(DWORD length, int gfp_flags)
{
  struct sk_buff *skb;
