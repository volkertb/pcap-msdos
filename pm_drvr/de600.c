static const char *version = "de600.c: $Revision: 1.40 $, "
...
static BYTE de600_read_byte (BYTE type, struct device *dev);

static int   de600_open (struct device *dev);
static void  de600_close (struct device *dev);
static void *get_stats (struct device *dev);
static int   de600_start_xmit (struct device *dev, const void *buf, int len);

static void  de600_interrupt (int irq);
 ...
 */
#define select_prn() outb(SELECT_PRN, COMMAND_PORT); DE600_SLOW_DOWN()
#define select_nic() outb(SELECT_NIC, COMMAND_PORT); DE600_SLOW_DOWN()

/* Thanks for hints from Mark Burton <markb@ordern.demon.co.uk>
 */
#define de600_put_byte(data) ( \
        outb (((data) << 4)   | WRITE_DATA            , DATA_PORT), \
        outb (((data) & 0xf0) | WRITE_DATA | HI_NIBBLE, DATA_PORT))

/*
 * The first two outb()'s below could perhaps be deleted if there
 * would be more delay in the last two. Not certain about it yet...
 */
#define de600_put_command(cmd) ( \
        outb(( rx_page        << 4)   | COMMAND            , DATA_PORT), \
        outb(( rx_page        & 0xf0) | COMMAND | HI_NIBBLE, DATA_PORT), \
        outb(((rx_page | cmd) << 4)   | COMMAND            , DATA_PORT), \
        outb(((rx_page | cmd) & 0xf0) | COMMAND | HI_NIBBLE, DATA_PORT))

#define de600_setup_address(addr,type) ( \
        outb((((addr) << 4) & 0xf0) | type            , DATA_PORT), \
        outb(( (addr)       & 0xf0) | type | HI_NIBBLE, DATA_PORT), \
        outb((((addr) >> 4) & 0xf0) | type            , DATA_PORT), \
        outb((((addr) >> 8) & 0xf0) | type | HI_NIBBLE, DATA_PORT))
...
static inline BYTE de600_read_status (struct device *dev)
{
  BYTE status;

  outb (STATUS, DATA_PORT);
  status = inb (STATUS_PORT);
  outb (NULL_COMMAND | HI_NIBBLE, DATA_PORT);
  return (status);
}

static inline BYTE de600_read_byte (BYTE type, struct device *dev)
{				/* dev used by macros */
  BYTE lo;

  outb ((type), DATA_PORT);
  lo = ((BYTE) inb (STATUS_PORT)) >> 4;
  outb ((type) | HI_NIBBLE, DATA_PORT);
  return ((BYTE) inb (STATUS_PORT) & (BYTE) 0xf0) | lo;
}
...
static int de600_start_xmit (struct device *dev, const void *buf, int len)
{
  ...
  {				/* Do timeouts, to avoid hangs. */
    ...
    printk ("%s: transmit timed out (%d), %s?\n",
            dev->name, (int)tickssofar, "network cable problem");
    /* Restart the adapter.
     */
    if (!adapter_init (dev))
       return (0);
  ...
  {				/* No transmission going on */
    ...
    dev->tx_busy  = 0;          /* allow more packets into adapter */
...
static void de600_interrupt (int irq)
{
   ...
   */
  if (!dev || !dev->start || DE600_IRQ != irq)
  {
...
static void de600_rx_intr (struct device *dev)
{
  ...
  if (dev->get_rx_buf)
  {
    char *buf = (*dev->get_rx_buf) (size);
    if (buf)
    {
      /* copy the packet into the buffer
       */
      de600_setup_address (read_from, RW_ADDR);
      for (i = size; i > 0; --i, ++buf)
          *buf = de600_read_byte (READ_DATA, dev);
    }
    else
...
int de600_probe (struct device *dev)
{
  ...
  /* Maybe we found one, have to check if it is a D-Link DE-600 adapter...
   *
  ...
  dev->open  = de600_open;
  dev->close = de600_close;
  dev->xmit  = de600_start_xmit;
