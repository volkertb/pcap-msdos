...
static void qe_init_rings(struct sunqe *qep, int from_irq)
{
  ...
  for(i = 0; i < RX_RING_SIZE; i++) {
    ...
    skb_reserve(skb, 34);
    qb->qe_rxd[i].rx_addr = (DWORD) skb->data;
...
static void sun4c_qe_init_rings(struct sunqe *qep)
{
  ...
  for(i = 0; i < SUN4C_RX_RING_SIZE; i++) {
    qb->qe_rxd[i].rx_addr = (DWORD) &qbufs->rx_buf[i][0];
    qb->qe_rxd[i].rx_flags =
...
static int qe_init(struct sunqe *qep, int from_irq)
{
  ...
  cregs->rxds = (DWORD) &qep->qe_block->qe_rxd[0];
  cregs->txds = (DWORD) &qep->qe_block->qe_txd[0];
 ...
 */
static int qe_is_bolixed(struct sunqe *qep, DWORD qe_status)
{
  struct device *dev = qep->dev;
  ...
  if(qe_status & CREG_STAT_CCOFLOW) {
    ...
    qep->net_stats.tx_collisions += 256;
  }
...
static inline void qe_rx(struct sunqe *qep)
{
  ...
  drop_it:
      /* Return it to the QE. */
      qep->net_stats.rx_dropped++;
      this->rx_addr = (DWORD) qep->rx_skbs[elem]->data;
      this->rx_flags =
    ...
    if(len > RX_COPY_THRESHOLD) {
      ...
      rxbase[elem].rx_addr = (DWORD) new_skb->data;
      ...
      eth_copy_and_sum(copy_skb, (BYTE *)skb->data, len, 0);

      /* Reuse original ring buffer. */
      rxbase[elem].rx_addr = (DWORD) skb->data;
      rxbase[elem].rx_flags =
        (RXD_OWN | ((RX_BUF_ALLOC_SIZE - 34) & RXD_LENGTH));
  ...
  while(!(this->rx_flags & RXD_OWN)) {
    ...
    if(len < ETH_ZLEN) {
      ...
      if(skb == 0) {
        ...
        skb_reserve(skb, 2);
        skb_put(skb, len);
        eth_copy_and_sum(skb, (BYTE *)this_qbuf,
             len, 0);
        skb->protocol = eth_type_trans(skb, qep->dev);
        netif_rx(skb);
        qep->net_stats.rx_packets++;
    ...
    end_rxd->rx_addr = (DWORD) this_qbuf;
...
static int qe_start_xmit(struct sk_buff *skb, struct device *dev)
{
  ...
  qep->qe_block->qe_txd[entry].tx_addr = (DWORD) skb->data;
...
static int sun4c_qe_start_xmit(struct sk_buff *skb, struct device *dev)
{
  ...
  qep->qe_block->qe_txd[entry].tx_addr = (DWORD) txbuf;
  qep->qe_block->qe_txd[entry].tx_flags =
    (TXD_OWN | TXD_SOP | TXD_EOP | (len & TXD_LENGTH));
  qep->tx_new = NEXT_TX(entry);
...
static void qe_set_multicast(struct device *dev)
{
  ...
  BYTE new_mconfig = (MREGS_MCONFIG_TXENAB | MREGS_MCONFIG_RXENAB);
  char *addrs;
  int i, j, bit, byte;
...
static int qec_ether_init(struct device *dev, struct linux_sbus_device *sdev)
{
  ...
  BYTE bsizes, bsizes_more, num_qranges;
  int i, j, res = ENOMEM;
  ...
  for(i = 0; i < 4; i++) {
    ...
    qe_devs[i]->irq = (BYTE) sdev->irqs[0].pri;
    qe_devs[i]->dma = 0;
