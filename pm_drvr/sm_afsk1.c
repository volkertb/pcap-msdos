...
static void modulator_1200(struct sm_state *sm, BYTE *buf, int buflen)
{
...
#define ENV_STORAGE BYTE fpu_save[108];
...
static inline int do_filter_1200(struct sm_state *sm, BYTE newval)
{
  float sum;
...
static inline void datamove8(signed char *st, BYTE newval)
{
  memmove(st+1, st, 7);
  *st = newval - 0x80;
}
...
static void demodulator_1200(struct sm_state *sm, BYTE *buf, int buflen)
{
