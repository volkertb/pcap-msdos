 ...
 *                     version 0.5 ALPHA
 * June 6, 1994 (ac)   Fixed to match the buffer locking changes. Added a
 *                     hack to fix a funny I see (search for HACK) and fixed
 *                     the calls in init() so it doesn't migrate module based
 *                     ethernet cards up to eth2 Took out the old module ideas
 *                     as they are no longer relevant to the PI driver.
 ...
 * Oct  29, 1995 (ac)  A couple of minor fixes before this, and this release
 *                     changes to the proper set_mac_address semantics which
 *                     will break a few programs I suspect.
 ...
 */
static int pi0_preprobe (struct device *dev)
{
  return 0;
}
...
static char *get_dma_buffer(DWORD * mem_ptr);
static int  valid_dma_page (DWORD addr, DWORD dev_buffsize);
...
static inline BYTE random (void)
{
  return (BYTE)(ext2_secrm_seed = ext2_secrm_seed * 69069l + 1);
}
...
static void a_txint (struct pi_local *lp)
{
  ...
  switch (lp->tstate)
  {
    case IDLE:
         ...
         /* If we have a buffer to send, we drop thru here
          */
...
static void a_exint (struct pi_local *lp)
{
  ...
  switch (lp->tstate)
  {
    ...
    case ST_TXDELAY:
         disable_dma (lp->dmachan);   /* Disable DMA chan */
          ...
          */
         wrtscc (lp->cardbase, cmd, R1,
                 WT_RDY_ENAB | WT_FN_RDYFN | EXT_INT_ENAB);
...
static void a_rxint (struct device *dev, struct pi_local *lp)
{
  ...
  {
    /* If end of frame
     * figure length of frame from 8237 DMA controller
     */
    ...
    else
    {
      ...
      *cfix++ = 0;
       ...
       */
      memcpy (cfix, cur_buf->data, pkt_len-1);
  ...
  switch (lp->tstate)
  {
    ...
    case ACTIVE:
         ...
         else
         {
           ...
           else wrtscc (lp->cardbase, cmd, R10, CRCPS);
...
static void b_exint (struct pi_local *lp)
{
  ...
  switch (lp->tstate)
  {
    ...
    case FLAGOUT:
          ...
          */
         if (lp->speed)       /* If internally clocked */
              wrtscc (lp->cardbase, cmd, R10, CRCPS | ABUNDER | NRZI );
         else wrtscc (lp->cardbase, cmd, R10, CRCPS | ABUNDER);
...
static int hw_probe (int ioaddr)
{
   ...
   * Set up counter chip. 500 uS square wave
   */
  outb_p (SC0 | LSB_MSB | MODE3, tmrcmd);
...
{
  DWORD flags;
  int   cardbase = dev->base_addr & 0x3f0;
  ...
  /* Hardware reset
   */
  wrtscc (cardbase, dev->base_addr + CTL, R9, FHWRES); 
...
int pi_init (void)
{
  ...
  printk (KERN_INFO "PI: V0.8 ALPHA April 23 1995 David Perry "
  ...
  pi0a.priv = kmalloc (sizeof(struct pi_local) +
                       (DMA_BUFF_SIZE + sizeof(struct mbuf)) * 4,
                       GFP_KERNEL | GFP_DMA);
  ...
  pi0b.priv = kmalloc (sizeof(struct pi_local) + (DMA_BUFF_SIZE +
                       sizeof(struct mbuf)) * 4, GFP_KERNEL | GFP_DMA);
...
static int pi_probe (struct device *dev, int card_type)
{
   ...
   */
  mem_ptr = (DWORD) dev->priv + sizeof (struct pi_local);
   ...
   */
  if (card_type == 1)
       lp->xtal = (DWORD) SINGLE / 2;
  else lp->xtal = (DWORD) DOUBLE / 2;
  ...
  if (dev->base_addr & CHANA)
  {
    /* Do these things only for the A port.
    ...
    if (dev->irq < 2)
    {
      ...
      /* Hardware reset
       */
      wrtscc (lp->cardbase, dev->base_addr + CTL, R9, FHWRES);
...
static int pi_ioctl (struct device *dev, struct ifreq *ifr, int cmd)
{
  ...
  struct pi_local *lp = (struct pi_local *) dev->priv;
  DWORD  flags;
  int    ret = verify_area (VERIFY_WRITE, ifr->ifr_data,
                            sizeof(struct pi_req));
  if (ret)
     return ret;
