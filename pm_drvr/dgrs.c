 ...
 */
#define S2H(A)   (((DWORD)(A) & 0x00ffffff) + priv0->vmem)
#define S2HN(A)  (((DWORD)(A) & 0x00ffffff) + privN->vmem)
 ...
 */
#define S2DMA(A) ((DWORD)(A) & 0x00ffffff)
...
BYTE dgrs_ipaddr[4]  = { 0xff, 0xff, 0xff, 0xff };
BYTE dgrs_iptrap[4]  = { 0xff, 0xff, 0xff, 0xff };
long dgrs_ipxnet     = -1;
int  dgrs_nicmode    = 0;
...
typedef struct {
  ...
  DWORD intrcnt;                /* Count of interrupts */

  /* SE-4 (EISA) board variables
   */
  BYTE is_reg;                 /* EISA: Value for ES4H_IS reg */
 ...
 */
static void proc_reset (struct device *dev0, int reset)
{
  DGRS_PRIV *priv0 = (DGRS_PRIV*) dev0->priv;

  if (priv0->plxreg)
  {
    ulong val = inl (dev0->base_addr + PLX_MISC_CSR);

    if (reset)
         val |=  SE6_RESET;
    else val &= ~SE6_RESET;
    _outportl (dev0->base_addr + PLX_MISC_CSR, val);
  }
  else
    _outportb (dev0->base_addr + ES4H_PC, reset ? ES4H_PC_RESET : 0);
}

/*
 *  See if the board supports bus master DMA
 */
static int check_board_dma (struct device *dev0)
{
  DGRS_PRIV *priv0 = (DGRS_PRIV*) dev0->priv;
  ulong      x;
   ...
   */
  _outportl (dev0->base_addr + PLX_ROM_BASE_ADDR, 0x80000000);
   ...
   *              0 internal wait states
   */
  _outportl (dev0->base_addr + PLX_BUS_REGION, 0x49430343);
 ...
 */
static int do_plx_dma (struct device *dev, ulong pciaddr, ulong lcladdr,
                       int len, int to_host)
{
  ...
  if (pciaddr)
  {
    ...
    priv->vplxdma[PLX_DMA0_DESCRIPTOR/4] = to_host ? PLX_DMA_DESC_TO_HOST
                                                   : PLX_DMA_DESC_TO_BOARD;
    priv->vplxdma[PLX_DMA0_MODE/4] = PLX_DMA_MODE_WIDTH32
  ...
  priv->vplxdma[PLX_DMA_CSR / 4] = PLX_DMA_CSR_0_ENABLE | PLX_DMA_CSR_0_START;
 ...
 */
static void dgrs_rcv_frame (struct device *dev0, DGRS_PRIV *priv0,
                            I596_CB *cbp)
{
  struct sk_buff *skb;
  struct device  *devN;
   ...
   */
  if (dgrs_nicmode)       /* Multi-NIC mode */
  {                      
    int chan = ((I596_RBD*) S2H (cbp->xmit.tbdp))->chan;
...
again:
  ...
  {
    /* If we can use DMA and its a long frame, copy it using DMA chaining.
     */
    ...
    for (;;)
    {
      ...
      {
        ddp_h->next = PLX_DMA_DESC_TO_HOST | PLX_DMA_DESC_EOC;
        ++ddp_h;
        break;
      }
      else
      {
        ++ddp_s;
        ddp_h->next = PLX_DMA_DESC_TO_HOST | (ulong) ddp_s;
    ...
    {
      if (do_plx_dma (dev0, 0, (ulong)priv0->dmadesc_s, len, 0))
      {
  ...
  else if (priv0->use_dma)
  {
    ...
    for (;;)
    {
      int count = tbdp->count;
      int amt   = count & 0x3fff;
      ...
      if (do_plx_dma (dev0, (ulong)phys_p, S2DMA(tbdp->buf), amt, 1))
      {
...
#define mymin(A,B)  ((A) < (B) ? (A) : (B))
  ...
  for (;;)
  {
    ...
    if (len == 0)
    {
      ...
      else rbdp->count = count | I596_RBD_EOF;
 ...
 *  Open the interface
 */
static int dgrs_open (struct device *dev)
{
  dev->tbusy = 0;
  dev->interrupt = 0;
  dev->start = 1;
...
static int dgrs_ioctl (struct device *devN, struct ifreq *ifr, int cmd)
{
  ...
  switch (ioc.cmd)
  {
    ...
    case DGRS_SETFILTER:
         ...
         if (ioc.len)
         {
           if (COPY_FROM_USER (S2HN (privN->bcomm->bc_filter_area), ioc.data, ioc.len))
             return -EFAULT;
...
ack_intr:
  if (priv0->plxreg)
    _outportl (dev0->base_addr + PLX_LCL2PCI_DOORBELL, 1);
}

/*
 *  Download the board firmware
 */
static int dgrs_download (struct device *dev0)
{
  DGRS_PRIV *priv0 = (DGRS_PRIV *) dev0->priv;
  int is;
  int i;
  ...
  {                             /* EISA bus */
    ...
    _outportb (dev0->base_addr + ES4H_AS_31_24, (BYTE) (dev0->mem_start >> 24));
    _outportb (dev0->base_addr + ES4H_AS_23_16, (BYTE) (dev0->mem_start >> 16));
    priv0->is_reg = ES4H_IS_LINEAR | is | ((BYTE) (dev0->mem_start >> 8) & ES4H_IS_AS15);
    _outportb (dev0->base_addr + ES4H_IS, priv0->is_reg);
    _outportb (dev0->base_addr + ES4H_EC, ES4H_EC_ENABLE);
    _outportb (dev0->base_addr + ES4H_PC, ES4H_PC_RESET);
    _outportb (dev0->base_addr + ES4H_MW, ES4H_MW_ENABLE | 0x00);
  }
  ...
  if (priv0->plxreg)
  {                             /* PCI bus */
    _outportl (dev0->base_addr + PLX_INT_CSR, inl (dev0->base_addr + PLX_INT_CSR) | PLX_PCI_DOORBELL_IE); /* Enable intr to host */
    _outportl (dev0->base_addr + PLX_LCL2PCI_DOORBELL, 1);
  }
  else
  {                             /* EISA bus */
 ...
 *  Probe (init) a board
 */
int dgrs_probe1 (struct device *dev)
{
  DGRS_PRIV *priv = (DGRS_PRIV *) dev->priv;
  int i;
  int rc;
  ...
  printk ("%s: Digi RightSwitch io=%lx mem=%lx irq=%d plx=%lx dma=%lx\n",
          dev->name, dev->base_addr, dev->mem_start, dev->irq, priv->plxreg, priv->plxdma);
   ...
   */
  if (priv->plxreg)
    _outportl (dev->base_addr + PLX_LCL2PCI_DOORBELL, 1);
...
int dgrs_initclone (struct device *dev)
{
  DGRS_PRIV *priv = (DGRS_PRIV *) dev->priv;
  int i;

  printk ("%s: Digi RightSwitch port %d ", dev->name, priv->chan);
  for (i = 0; i < 6; ++i)
    printk ("%c%2.2x", i ? ':' : ' ', dev->dev_addr[i]);
  printk ("\n");
...
static int dgrs_found_device (struct device *dev, int io, ulong mem, int irq, ulong plxreg, ulong plxdma)
{
  DGRS_PRIV *priv;
  int        i;
 ...
 *  Scan for all boards
 */
static int dgrs_scan (struct device *dev)
{
  int  cards_found = 0;
  uint io;
  uint mem;
  ...
  if (pcibios_present())
  {
    ...
    for (; pci_index < 8; pci_index++)
    {
      ...
      if (pcibios_find_device (SE6_PCI_VENDOR_ID, SE6_PCI_DEVICE_ID,
                               pci_index, &pci_bus, &pci_device_fn))
        break;

      pcibios_read_config_byte (pci_bus, pci_device_fn, PCI_INTERRUPT_LINE, &pci_irq);
      pcibios_read_config_dword(pci_bus, pci_device_fn, PCI_BASE_ADDRESS_0, &plxreg);
      pcibios_read_config_dword(pci_bus, pci_device_fn, PCI_BASE_ADDRESS_1, &io);
      pcibios_read_config_dword(pci_bus, pci_device_fn, PCI_BASE_ADDRESS_2, &mem);
      pcibios_read_config_dword(pci_bus, pci_device_fn, 0x30, &plxdma);
      irq     = pci_irq;
      plxreg &= ~15;
      io     &= ~3;
      mem    &= ~15;
       ...
       */
      _outportl (io + PLX_SPACE0_RANGE, 0xFFE00000L);
      if (plxdma == 0)
         plxdma = mem + (2048L * 1024L);
      pcibios_write_config_dword(pci_bus, pci_device_fn, 0x30, plxdma + 1);
      pcibios_read_config_dword (pci_bus, pci_device_fn, 0x30, &plxdma);
      plxdma &= ~15;
      ...
      if (pci_latency != 255)
      {
        printk ("  Overriding PCI latency timer: was %d, now is 255.\n",
                pci_latency);
...
int dgrs_probe (struct device *dev)
{
  int cards_found = dgrs_scan (dev);
