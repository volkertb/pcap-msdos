...
static enum uart ser12_check_uart(DWORD iobase)
{
  BYTE b1,b2,b3;
  enum uart u;
  enum uart uart_tab[] =
...
{
  int i;
  DWORD data = hdlcdrv_getbits(&bc->hdrv);

  for(i = 0; i < PAR96_BURSTBITS; i++, data >>= 1) {
    BYTE val = PAR97_POWER;
...
static int par96_check_lpt(DWORD iobase)
{
  BYTE b1,b2;
  int i;
