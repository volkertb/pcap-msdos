...
#define  toff(t,p,f)   (WORD)((void *)(&((t *)((void *)0 + (p)))->f) - (void *)0)
...
struct scp_t
{
  WORD  scp_sysbus;  /* 82586 bus width:  */
#define    SCP_SY_16BBUS  (0x0 << 0)  /* 16 bits */
#define    SCP_SY_8BBUS  (0x1 << 0)  /*  8 bits. */
  WORD  scp_junk[2];  /* Unused */
  WORD  scp_iscpl;  /* lower 16 bits of ISCP_ADDR */
  WORD  scp_iscph;  /* upper 16 bits of ISCP_ADDR */
};
...
struct iscp_t
{
  WORD  iscp_busy;  /* set by CPU before first CA,  */
          /* cleared by 82586 after read.  */
  WORD  iscp_offset;  /* offset of SCB    */
  WORD  iscp_basel;  /* base of SCB      */
  WORD  iscp_baseh;  /*  "        */
};
...
struct scb_t
{
  WORD  scb_status;  /* Status of 82586    */
...
#define      SCB_CMD_RUC_ABT  (4 << 4)  /* Abort     "  */
  WORD  scb_cbl_offset;  /* Offset of first command unit  */
          /* Action Command    */
  WORD  scb_rfa_offset;  /* Offset of first Receive  */
          /* Frame Descriptor in the  */
          /* Receive Frame Area    */
  WORD  scb_crcerrs;  /* Properly aligned frames  */
          /* received with a CRC error  */
  WORD  scb_alnerrs;  /* Misaligned frames received  */
          /* with a CRC error    */
  WORD  scb_rscerrs;  /* Frames lost due to no space  */
  WORD  scb_ovrnerrs;  /* Frames lost due to slow bus  */
};
...
#define    AC_SFLD_MAXCOL  (0xF <<  0)  /* Collision count    */
  WORD  ac_command;    /* Command specifier:  */
...
#define    AC_CFLD_CMD  (0x7 <<  0)  /* acmd_e    */
  WORD  ac_link;    /* Next Action Command  */
};
...
{
  ach_t    ias_h;
  BYTE  ias_addr[ADDR_LEN]; /* The (ethernet) address  */
};
...
{
  ach_t    cfg_h;
  BYTE  cfg_byte_cnt;  /* Size foll data: 4-12  */
#define  AC_CFG_BYTE_CNT(v)  (((v) & 0xF) << 0)
  BYTE  cfg_fifolim;  /* FIFO threshold  */
...
#define  AC_CFG_LINPRIO(v)  (((v) & 0x7) << 0)  /* Linear priority  */
  BYTE  cfg_ifs;  /* Interframe spacing    */
  BYTE  cfg_slotl;  /* Slot time (low byte)    */
  BYTE  cfg_byte13;
...
{
  ach_t    mcs_h;
  WORD  mcs_cnt;  /* No. of bytes of MC addresses  */
#if 0
  BYTE  mcs_data[ADDR_LEN]; /* The first MC address ..  */
  ...
#endif
};
...
{
  ach_t    tx_h;
  WORD  tx_tbd_offset;  /* Address of list of buffers.  */
#if  0
...
ac_cfg_t action command.
  BYTE  tx_addr[ADDR_LEN]; /* The frame dest. address  */
  WORD  tx_length;  /* The frame length    */
#endif  /* 0 */
};
...
struct tbd_t
{
  WORD  tbd_status;    /* Written by the CPU  */
#define    TBD_STATUS_EOF  (0x1 << 15)  /* This TBD is the  */
            /* last for this frame  */
#define    TBD_STATUS_ACNT  (0x3FFF << 0)  /* Actual count of data  */
            /* bytes in this buffer  */
  WORD  tbd_next_bd_offset;  /* Next in list    */
  WORD  tbd_bufl;    /* Buffer address (low)  */
  WORD  tbd_bufh;    /*    "       "  (high)  */
};
...
struct rbd_t
{
  WORD  rbd_status;    /* Written by the 82586  */
...
#define    RBD_STATUS_ACNT  (0x3FFF << 0)  /* Actual no. of data  */
            /* bytes in this buffer  */
  WORD  rbd_next_rbd_offset;  /* Next rbd in list  */
  WORD  rbd_bufl;    /* Data pointer (low)  */
  WORD  rbd_bufh;    /*  "     "    (high)  */
  WORD  rbd_el_size;    /* EL+Data buf. size  */
...
struct fd_t
{
  WORD  fd_status;    /* Written by the 82586  */
...
#endif  /* 0 */
  BYTE  fd_dest[ADDR_LEN];  /* Destination address  */
            /* Written by 82586  */
  BYTE  fd_src[ADDR_LEN];  /* Source address  */
            /* Written by 82586  */
  WORD  fd_length;    /* Frame length or type  */
            /* Written by 82586  */
};
