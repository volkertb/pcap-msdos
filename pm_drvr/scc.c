...
static void t_dwait(DWORD);
static void t_txdelay(DWORD);
static void t_tail(DWORD);
static void t_busy(DWORD);
static void t_maxkeyup(DWORD);
static void t_idle(DWORD);
static void scc_tx_done(struct scc_channel *);
static void scc_start_tx_timer(struct scc_channel *, void (*)(DWORD), DWORD);
...
static int scc_net_setup(struct scc_channel *scc, BYTE *name);
...
static int scc_net_header(struct sk_buff *skb, struct device *dev, WORD type, void *daddr, void *saddr, unsigned len);
static struct net_device_stats * scc_net_get_stats(struct device *dev);

static BYTE *SCC_DriverName = "scc";

static struct irqflags { BYTE used : 1; } Ivec[16];
...
static BYTE Driver_Initialized = 0;
static int Nchips = 0;
...
extern __inline__ BYTE InReg(io_port port, BYTE reg)
{
  DWORD flags;
  BYTE r;
...
extern __inline__ void OutReg(io_port port, BYTE reg, BYTE val)
{
  DWORD flags;
...
extern __inline__ void wr(struct scc_channel *scc, BYTE reg,
  BYTE val)
{
  OutReg(scc->ctrl, reg, (scc->wreg[reg] = val));
}

extern __inline__ void or(struct scc_channel *scc, BYTE reg, BYTE val)
{
  OutReg(scc->ctrl, reg, (scc->wreg[reg] |= val));
}

extern __inline__ void cl(struct scc_channel *scc, BYTE reg, BYTE val)
{
  OutReg(scc->ctrl, reg, (scc->wreg[reg] &= ~val));
}
...
{
  BYTE status,changes,chg_and_stat;
...
extern __inline__ void set_brg(struct scc_channel *scc, DWORD tc)
{
...
static void scc_start_tx_timer(struct scc_channel *scc, void (*handler)(DWORD), DWORD when)
{
  DWORD flags;
...
static void t_dwait(DWORD channel)
{
...
static void t_txdelay(DWORD channel)
{
  struct scc_channel *scc = (struct scc_channel *) channel;
...
static void t_tail(DWORD channel)
{
  struct scc_channel *scc = (struct scc_channel *) channel;
  DWORD flags;
...
static void t_busy(DWORD channel)
{
  struct scc_channel *scc = (struct scc_channel *) channel;
  DWORD flags;
...
static void t_maxkeyup(DWORD channel)
{
  struct scc_channel *scc = (struct scc_channel *) channel;
  DWORD flags;
...
static void t_idle(DWORD channel)
{
  struct scc_channel *scc = (struct scc_channel *) channel;
  DWORD flags;
...
static DWORD scc_set_param(struct scc_channel *scc, DWORD cmd, DWORD arg)
{
  int dcd;
...
static DWORD scc_get_param(struct scc_channel *scc, DWORD cmd)
{
  switch (cmd)
  {
...
static int scc_net_setup(struct scc_channel *scc, BYTE *name)
{
  ...
  buf = (BYTE *) kmalloc(10, GFP_KERNEL);
  strcpy(buf, name);
...
static BYTE ax25_bcast[AX25_ADDR_LEN] =
{'Q' << 1, 'S' << 1, 'T' << 1, ' ' << 1, ' ' << 1, ' ' << 1, '0' << 1};
static BYTE ax25_nocall[AX25_ADDR_LEN] =
