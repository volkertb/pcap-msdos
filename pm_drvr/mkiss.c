...
static int mkiss_write(struct tty_struct *, int, const BYTE *, int);
static int kiss_esc(BYTE *, BYTE *, int);
static void kiss_unesc(struct ax_disp *, BYTE);
...
static void ax_encaps(struct ax_disp *ax, BYTE *icp, int len)
{
  BYTE *p;
  int actual, count;
  ...
  if (mkiss->magic  != MKISS_DRIVER_MAGIC) {
    count = kiss_esc(p, (BYTE *)ax->xbuff, len);
    ...
    count = kiss_esc(p, (BYTE *) ax->mkiss->xbuff, len);
...
static int ax_header(struct sk_buff *skb, struct device *dev, WORD type,
...
static int ax_open(struct device *dev)
{
  ...
  if ((ax->rbuff = (BYTE *) kmalloc(len + 4, GFP_KERNEL)) == NULL)
    goto norbuff;

  if ((ax->xbuff = (BYTE *) kmalloc(len + 4, GFP_KERNEL)) == NULL)
    goto noxbuff;
 ...
 */
static void ax25_receive_buf(struct tty_struct *tty, const BYTE *cp, char *fp, int count)
{
  struct ax_disp *ax = (struct ax_disp *)tty->disc_data;
...
int kiss_esc(BYTE *s, BYTE *d, int len)
{
  BYTE *ptr = d;
  BYTE c;
...
static void kiss_unesc(struct ax_disp *ax, BYTE s)
{
  switch (s) {
    case END:
...
int mkiss_init_ctrl_dev(void)
{
  ...
  ax_ldisc.read   = NULL;
  ax_ldisc.write  = NULL;
  ax_ldisc.ioctl  = (int (*)(struct tty_struct *, struct file *, DWORD, DWORD))ax25_disp_ioctl;
  ax_ldisc.poll   = NULL;
...
static int mkiss_write(struct tty_struct *tty, int from_user, const BYTE *buf, int count)
{
  return 0;
}

static int mkiss_ioctl(struct tty_struct *tty, struct file *file, DWORD cmd, DWORD arg)
{
...
static void mkiss_dummy2(struct tty_struct *tty, BYTE ch)
{
  struct mkiss_channel *mkiss = tty->driver_data;
