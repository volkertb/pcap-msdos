...
struct bridge_mapping_type {
       BYTE  addr;           /* config space address */
       BYTE  mask;
       BYTE  value;
     } bridge_mapping[] = {
       /*
 ...
 * __initfunc
 */
static void quirk_bridge (struct pci_dev *dev, int pos)
{
  struct bridge_mapping_type *bmap;
  BYTE   val;
  int    i;

  pos *= DIM(bridge_optimization);
  for (i = 0; i < DIM(bridge_optimization); i++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        pci_write_config_byte (dev, bmap->addr,
 ...
 * __initfunc
 */
static void quirk_passive_release (struct pci_dev *dev, int arg)
{
   ...
   */
  while ((d = pci_find_device (PCI_VENDOR_ID_INTEL,
                               PCI_DEVICE_ID_INTEL_82371SB_0, d)) != NULL)
  {
 ...
 * __initfunc
 */
static void quirk_isa_dma_hangs (struct pci_dev *dev, int arg)
{
  if (!isa_dma_bridge_buggy)
  {
    isa_dma_bridge_buggy = 1;
    PRINTK (("Activating ISA DMA hang workarounds.\n"));
  }
}
...
static struct quirk_name quirk_names[] = {
#ifdef CONFIG_PCI_OPTIMIZE
...
static char *get_quirk_name (quirk_handler handler)
{
  int i;

  for (i = 0; i < DIM(quirk_names); i++)
...
/*
 * Mapping from PCI vendor/device ID pairs to quirk function types
 * and arguments
 */
struct quirk_info {
       WORD          vendor;
...
static struct quirk_info quirk_list[] = {
#ifdef CONFIG_PCI_OPTIMIZE
