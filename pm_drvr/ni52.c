...
#define make16(ptr32) ((WORD) ((DWORD)(ptr32) - (DWORD) p->memtop ))
...
static void    ni52_close(struct device *dev);
...
static int ni52_open (struct device *dev)
{
  ...
  if (!request_irq(dev->irq, &ni52_interrupt))
  {
    irq2dev_map[dev->irq] = NULL;
    ni_reset586();
    return (0);
...
static int check586 (struct device *dev,char *where,unsigned size)
{
  ...
  p->base = (DWORD) bus_to_virt((DWORD)where) + size - 0x01000000;
  p->memtop = bus_to_virt((DWORD)where) + size;
  ...
  iscp_addrs[0] = bus_to_virt((DWORD)where);
...
static int ni52_probe1(struct device *dev,int ioaddr)
{
  ...
  ((struct priv *) (dev->priv))->base =  (DWORD) bus_to_virt(dev->mem_start) + size - 0x01000000;
  alloc586(dev);
      ...
      if(status & RFD_OK) /* frame received without error? */
      {
        if( (totlen = rbd->status) & RBD_LAST) /* the first and the last buffer? */
        {
          ...
          if(skb != NULL)
          {
            ...
            skb_reserve(skb,2);
            skb_put(skb,totlen);
            eth_copy_and_sum(skb,(char *) p->base+(DWORD) rbd->buffer,totlen,0);
...
static void ni52_xmt_int(struct device *dev)
{
  ...
  {
    p->stats.tx_packets++;
    p->stats.tx_collisions += (status & TCMD_MAXCOLLMASK);
  }
  else
  {
    p->stats.tx_errors++;
...
void ni52_dump(struct device *dev,void *ptr)
{
  ...
  for(i=0;i<170;i++) {
    printk("%02x ",(int) ((BYTE *) (dump_cmd + 1))[i]);
    if(i % 24 == 23)
      printk("\n");
  }
  printk("\n");
