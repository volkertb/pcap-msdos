...
struct net_local {
       ...
       WORD   saved_tx_size;
       BYTE   re_tx;             /* Number of packet retransmissions. */
       BYTE   tx_unit_busy;
       BYTE   addr_mode;         /* Current Rx filter e.g. promiscuous, etc. */
       BYTE   pac_cnt_in_tx_buf;
     };

struct rx_header {
       WORD pad;                 /* The first read is always corrupted. */
       WORD rx_count;
       WORD rx_status;           /* Unknown bit assignments :-<.  */
       WORD cur_addr;            /* Apparently the current buffer address(?) */
     };
 ...
 */
extern inline BYTE inbyte (WORD port)
{
  BYTE _v;
  __asm__ __volatile__ (
           "inb %w1,%b0":"=a" (_v):"d" (port));
  return (_v);
}
 ...
 */
extern inline BYTE read_nibble (short port, BYTE offset)
{
  BYTE retval;
 ...
 */
extern inline BYTE read_byte_mode0 (short ioaddr)
{
  BYTE low_nib;
 ...
 */
extern inline BYTE read_byte_mode2 (short ioaddr)
{
  BYTE low_nib;
 ...
 */
extern inline BYTE read_byte_mode4 (short ioaddr)
{
  BYTE low_nib;
 ...
 */
extern inline BYTE read_byte_mode6 (short ioaddr)
{
  BYTE low_nib;
...
extern inline void write_reg (short port, BYTE reg, BYTE value)
{
  BYTE outval;
...
extern inline void write_reg_high (short port, BYTE reg, BYTE value)
{
  BYTE outval = EOC | HNib | reg;
 ...
 */
extern inline void write_reg_byte (short port, BYTE reg, BYTE value)
{
  BYTE outval;
 ...
 * lines.
 */
extern inline void write_byte_mode0 (short ioaddr, BYTE value)
{
  outb (value & 0x0f, ioaddr + PAR_DATA);
  outb ((value >> 4) | 0x10, ioaddr + PAR_DATA);
}

extern inline void write_byte_mode1 (short ioaddr, BYTE value)
{
  outb (value & 0x0f, ioaddr + PAR_DATA);
 ...
 */
extern inline void write_word_mode0 (short ioaddr, WORD value)
{
  outb (value & 0x0f, ioaddr + PAR_DATA);
  value >>= 4;
