...
void lance32_probe1(struct device *dev, char *chipname, int pci_irq_line)
{
  ...
  lp = (struct lance32_private *) (((DWORD)kmalloc(sizeof(*lp)+15, GFP_DMA | GFP_KERNEL)+15) & ~15);
      
  memset(lp, 0, sizeof(*lp));
  dev->priv = lp;
  lp->name = chipname;
  lp->rx_buffs = (DWORD) kmalloc(PKT_BUF_SZ*RX_RING_SIZE, GFP_DMA | GFP_KERNEL);
...
static void
lance32_restart(struct device *dev, DWORD csr0_bits, int must_reinit)
{
        int i;
  int ioaddr = dev->base_addr;
    ...
    else 
    {
      ...
      else
      {
        ...
        eth_copy_and_sum(skb,
          (BYTE *)bus_to_virt(lp->rx_ring[entry].base),
          pkt_len,0);
