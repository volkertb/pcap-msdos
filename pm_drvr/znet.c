...
#define ZNET_DEBUG 1
#endif
static DWORD znet_debug = ZNET_DEBUG;
...
struct netidblk {
  ...
  BYTE netid[8]; /* The physical station address */
...
static void update_stop_hit(short ioaddr, WORD rx_stop_offset);
...
static void znet_rx(struct device *dev)
{
  ...
  /* Use same method as the Crynwr driver: construct a forward list in
   ...
   while (zn.rx_start + cur_frame_end_offset != zn.rx_cur
      && ++boguscount < 5) {
    WORD hi_cnt, lo_cnt, hi_status, lo_status;
    int count, status;
...
static void hardware_init(struct device *dev)
{
  ...
  cli(); {              /* Protect against a DMA flip-flop */
    ...
    set_dma_addr(zn.rx_dma, (DWORD) zn.rx_start);
    ...
    set_dma_addr(zn.tx_dma, (DWORD) zn.tx_start);
