...
static const char *version = "at1700.c:v1.12 1/18/95  Donald Becker (becker@cesdis.gsfc.nasa.gov)\n";

#include "pmdrvr.h"

/* This unusual address order is used to verify the CONFIG register.
 */
static int at1700_probe_list[] = { 0x260, 0x280, 0x2a0, 0x240,
                                   0x340, 0x320, 0x380, 0x300, 0
                                 };
...
#define AT17_DEBUG 1
#endif
int at17_debug = AT17_DEBUG;

/* Information that need to be kept for each board.
 */
struct net_local {
       struct net_device_stats stats;
       BOOL   tx_started:1;         /* Number of packet on the Tx queue. */
...
static void  net_receive (struct device *dev);
...
int at1700_probe1 (struct device *dev, short ioaddr)
{
...
#endif
  if (at1700_probe_list [inb(ioaddr+IOCONFIG) & 7] != ioaddr ||
      read_eeprom (ioaddr, 4) != 0 ||
  ...
  irq = irqmap [(read_eeprom(ioaddr,12) & 4) |
  ...
  printk ("%s: AT1700 found at %3x, IRQ %d, address ",
          dev->name, ioaddr, irq);
  ...
  {
    WORD eeprom_val = read_eeprom (ioaddr, 4+i);

    printk ("%04x", eeprom_val);
    ((WORD *) dev->dev_addr)[i] = ntohs (eeprom_val);
  }
   ...
   */
  {
    const char *porttype[] = { "auto-sense", "10baseT",
                               "auto-sense", "10base2"
                             };
    WORD  setup_value = read_eeprom (ioaddr, 12);
   ...
   * 16 bit bus access, two 4K Tx queues, and disabled Tx and Rx.
   */
...
static int read_eeprom (int ioaddr, int location)
{
  ...
  int   i, read_cmd = location | EE_READ_CMD;
...
static int net_open (struct device *dev)
{
  ...
  /* Set the configuration register 0 to 32K 100ns. byte-wide memory,
   * 16 bit bus access, and two 4K Tx queues.
   */
  outb (0xda, ioaddr + CONFIG_0);
  ...
  /* Set receive mode from dev->flags
   */
...
static int net_send_packet (struct device *dev, const void *buf, int len)
{
  ...
  if (dev->tx_busy)
  {
    ...
    printk ("%s: transmit timed out with status %04x, %s?\n",
            ...
            inb(ioaddr + TX_STATUS) & 0x80 ? "IRQ conflict"
                                           : "network cable problem");
    printk ("%s: timeout registers: %04x %04x %04x %04x %04x %04x %04x %04x.\n",
            dev->name, inw(ioaddr+0), inw(ioaddr+2), inw(ioaddr+4),
            inw(ioaddr+6), inw(ioaddr+8), inw(ioaddr+10), inw(ioaddr+12),
            inw(ioaddr+14));

    lp->stats.tx_errors++;
    /* ToDo: We should try to restart the adapter...
     */
...
static void net_interrupt (int irq)
{
  ...
  if (status & 0xff00 ||
      (inb (ioaddr + RX_MODE) & 0x40) == 0)  /* Got packet(s). */
     net_receive (dev);
...
static void net_receive (struct device *dev)
{
  ...
  while ((inb (ioaddr + RX_MODE) & 0x40) == 0)
  {
    ...
    WORD length = inw (ioaddr + DATAPORT);
	...
	break;
      ...
      if (dev->get_rx_buf)
      {
        char *buf = (*dev->get_rx_buf) (length);
        if (buf)
             rep_insw (ioaddr + DATAPORT, (WORD*)buf, (length+1) >> 1);
  ...
  if (at17_debug > 5)
     printk ("%s: Exint Rx packet with mode %02x after %d loops.\n",
...
#if 0
  DISABLE();
  /* to-do: Update the statistics from the device registers.
   */
  ENABLE();
#endif
