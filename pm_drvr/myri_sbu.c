...
static void myri_init_rings(struct myri_eth *mp, int from_irq)
{
  ...
  for(i = 0; i < RX_RING_SIZE; i++) {
    ...
    rxd[i].myri_scatters[0].addr = (DWORD) skb->data;
...
static void dump_ehdr_and_myripad(BYTE *stuff)
{
 ...
 */
static WORD myri_type_trans(struct sk_buff *skb, struct device *dev)
{
  struct ethhdr *eth;
  BYTE *rawp;
  
  skb->mac.raw = (((BYTE *)skb->data) + MYRI_PAD_LEN);
...
static inline void myri_rx(struct myri_eth *mp, struct device *dev)
{
  ...
  drop_it:
      ...
      rxd->myri_scatters[0].addr = (DWORD) skb->data;
    ...
    if(len > RX_COPY_THRESHOLD) {
      ...
      rxd->myri_scatters[0].addr = (DWORD) new_skb->data;
      rxd->myri_scatters[0].len = RX_ALLOC_SIZE;
      rxd->ctx = index;
      rxd->num_sg = 1;
      rq->tail = NEXT_RX(rq->tail);
...
static int myri_start_xmit(struct sk_buff *skb, struct device *dev)
{
  ...
  DHDR(("xmit[skbdata(%p)]\n", skb->data));
#ifdef DEBUG_HEADER
  dump_ehdr_and_myripad(((BYTE *) skb->data));
#endif
  ...
  txd->myri_gathers[0].addr = (DWORD) skb->data;
  txd->myri_gathers[0].len = len;
  txd->num_sg = 1;
  txd->chan = KERNEL_CHANNEL;
  txd->len = len;
  txd->csum_off = ((DWORD)-1);
  txd->csum_field = 0;

  srcptr = (((BYTE *) skb->data) + MYRI_PAD_LEN);
  if(srcptr[0] & 0x1) {
 ...
 */
static int myri_header(struct sk_buff *skb, struct device *dev, WORD type,
           void *daddr, void *saddr, unsigned len)
{
  struct ethhdr *eth = (struct ethhdr *)skb_push(skb,ETH_HLEN);
  BYTE *pad = (BYTE *)skb_push(skb,MYRI_PAD_LEN);
...
int myri_header_cache(struct dst_entry *dst, struct dst_entry *neigh,
           struct hh_cache *hh)
{
  WORD type = hh->hh_type;
  BYTE *pad = (BYTE *)hh->hh_data;
  struct ethhdr *eth = (struct ethhdr *)(pad + MYRI_PAD_LEN);
...
static int myri_ether_init(struct device *dev, struct linux_sbus_device *sdev, int num)
{
  ...
  if(mp->eeprom.cpuvers < CPUVERS_4_0) {
    ...
    mp->lanai = (WORD *) (((DWORD)mp->regs) + (256*1024));
    mp->lanai3 = (DWORD *) mp->lanai;
