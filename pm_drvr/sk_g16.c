...
typedef struct
{
  BYTE  ram[0x3fc0];   /* 16KB dual ported ram */
  BYTE  rom[0x0020];   /* 32Byte PROM containing 6Byte MAC */
  BYTE  res1[0x0010];  /* reserved */
  unsigned volatile short ioreg;/* LANCE I/O Register */
  unsigned volatile char  port; /* Statusregister and Controlregister */
  BYTE  iocom;         /* I/O Command Register */
} SK_RAM;
...
static int SK_lance_init(struct device *dev, WORD mode);
...
int SK_probe(struct device *dev, short ioaddr)
{
    ...
    DWORD rom_addr;  /* used to store RAM address used for POS_ADDR */
...
static int SK_open(struct device *dev)
{
  ...
  outb(i<<2, SK_POS4);           /* Set IRQ on card */
    ...
    printk("%s: Schneider & Koch G16 at %#3x, IRQ %d, shared mem at %#08x\n",
      dev->name, (DWORD)dev->base_addr, 
      (int) dev->irq, (DWORD) p->ram);
...
static int SK_lance_init(struct device *dev, WORD mode)
{
  ...
  tmdp->u.buffer = (DWORD) p->tmdbufs[i]; /* assign buffer */
  ...
  rmdp->u.buffer = (DWORD) p->rmdbufs[i]; /* assign buffer */
...
static void SK_rxintr(struct device *dev)
{
  ...
  else /* We have a packet which can be queued for the upper layers */
  {
    ...
    break;                      /* Jump out */
      ...
      memcpy(skb_put(skb,len), (BYTE *) (rmdp->u.buffer & 0x00ffffff),
       len);
...
DWORD SK_rom_addr(void)
{
    int i,j;
    int rom_found = 0;
    DWORD rom_location[] = SK_BOOT_ROM_LOCATIONS;
    BYTE rom_id[] = SK_BOOT_ROM_ID;
    BYTE *test_byte;
  ...
  for (j = 0; j < 6; j++)
  {
      test_byte = (BYTE *) (rom_location[i]+j);
