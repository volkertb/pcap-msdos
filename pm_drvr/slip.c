...
static int slip_esc(BYTE *p, BYTE *d, int len);
static void slip_unesc(struct slip *sl, BYTE c);
#ifdef CONFIG_SLIP_MODE_SLIP6
static int slip_esc6(BYTE *p, BYTE *d, int len);
static void slip_unesc6(struct slip *sl, BYTE c);
#endif
#ifdef CONFIG_SLIP_SMART
static void sl_keepalive(DWORD sls);
static void sl_outfill(DWORD sls);
#endif
...
static void sl_changedmtu(struct slip *sl)
{
  ...
  BYTE *xbuff, *rbuff, *oxbuff, *orbuff;
#ifdef SL_INCLUDE_CSLIP
  BYTE *cbuff, *ocbuff;
#endif
  int len;
 ...
 */
  ...
  xbuff = (BYTE *) kmalloc (len + 4, GFP_ATOMIC);
  rbuff = (BYTE *) kmalloc (len + 4, GFP_ATOMIC);
#ifdef SL_INCLUDE_CSLIP
  cbuff = (BYTE *) kmalloc (len + 4, GFP_ATOMIC);
#endif
...
static void
sl_encaps(struct slip *sl, BYTE *icp, int len)
{
  BYTE *p;
  int actual, count;
  ...
  if(sl->mode & SL_MODE_SLIP6)
    count = slip_esc6(p, (BYTE *) sl->xbuff, len);
  else
#endif
    count = slip_esc(p, (BYTE *) sl->xbuff, len);
...
sl_open(struct device *dev)
{
  ...
  sl->rbuff = (BYTE *) kmalloc(len + 4, GFP_KERNEL);
  if (sl->rbuff == NULL)   {
    goto norbuff;
  }
  sl->xbuff = (BYTE *) kmalloc(len + 4, GFP_KERNEL);
  if (sl->xbuff == NULL)   {
    goto noxbuff;
  }
#ifdef SL_INCLUDE_CSLIP
  sl->cbuff = (BYTE *) kmalloc(len + 4, GFP_KERNEL);
  if (sl->cbuff == NULL)   {
    goto nocbuff;
  }
  ...
  sl->keepalive_timer.data=(DWORD)sl;
  sl->keepalive_timer.function=sl_keepalive;
  sl->outfill=0;      /* & outfill too */
  init_timer(&sl->outfill_timer);
  sl->outfill_timer.data=(DWORD)sl;
...
static void slip_receive_buf(struct tty_struct *tty, const BYTE *cp, char *fp, int count)
{
  struct slip *sl = (struct slip *) tty->disc_data;
...
sl_get_stats(struct device *dev)
{
  ...
  stats.tx_collisions  = sl->tx_misses;
  comp = sl->slcomp;
  if (comp) {
    stats.rx_fifo_errors += comp->sls_i_compressed;
    stats.rx_dropped     += comp->sls_i_tossed;
    stats.tx_fifo_errors += comp->sls_o_compressed;
    stats.tx_collisions  += comp->sls_o_misses;
  }
#endif /* CONFIG_INET */
  return (&stats);
}
...
int
slip_esc(BYTE *s, BYTE *d, int len)
{
  BYTE *ptr = d;
  BYTE c;
...
static void slip_unesc(struct slip *sl, BYTE s)
{
...
int
slip_esc6(BYTE *s, BYTE *d, int len)
{
  BYTE *ptr = d;
  BYTE c;
  int i;
...
void
slip_unesc6(struct slip *sl, BYTE s)
{
  BYTE c;
...
slip_ioctl(struct tty_struct *tty, void *file, int cmd, void *arg)
{
  ...
  case TCGETS:
  case TCGETA:
    return n_tty_ioctl(tty, (struct file *) file, cmd, (DWORD) arg);
...
#ifdef CONFIG_SLIP_MODE_SLIP6
      BYTE s = (sl->mode & SL_MODE_SLIP6)?0x70:END;
#else
      BYTE s = END;
#endif
