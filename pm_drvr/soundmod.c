...
struct modem_tx_info {
  ...
  int srate;
  int bitrate;
  DWORD dmabuflenmodulo;
  void (*modulator)(struct sm_state *, BYTE *, int);
...
struct modem_rx_info {
  ...
  DWORD dmabuflenmodulo;
  DWORD sperbit;
  void (*demodulator)(struct sm_state *, BYTE *, int);
  void (*init)(struct sm_state *);
};
...
static int check_lpt(DWORD iobase)
{
  BYTE b1,b2;
  int i;
...
static enum uart check_uart(DWORD iobase)
{
  BYTE b1,b2,b3;
  enum uart u;
  enum uart uart_tab[] =
...
static int check_midi(DWORD iobase)
{
  DWORD timeout;
  DWORD flags;
  BYTE b;
...
extern inline DWORD hweight32(DWORD w)
  __attribute__ ((unused));
extern inline DWORD hweight16(WORD w)
  __attribute__ ((unused));
extern inline DWORD hweight8(BYTE w)
        __attribute__ ((unused));

extern inline DWORD hweight32(DWORD w)
{
        DWORD res = (w & 0x55555555) + ((w >> 1) & 0x55555555);
...
extern inline DWORD hweight8(BYTE w)
{
        WORD res = (w & 0x55) + ((w >> 1) & 0x55);
        res = (res & 0x33) + ((res >> 2) & 0x33);
        return (res & 0x0F) + ((res >> 4) & 0x0F);
}

extern inline DWORD gcd(DWORD x, DWORD y)
  __attribute__ ((unused));
extern inline DWORD lcm(DWORD x, DWORD y)
  __attribute__ ((unused));
