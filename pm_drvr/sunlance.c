...
struct lance_rx_desc {
  WORD rmd0;        /* low address of packet */
  BYTE  rmd1_bits;   /* descriptor bits */
  BYTE  rmd1_hadr;   /* high address of packet */
  short    length;          /* This length is 2s complement (negative)!
             * Buffer length
             */
  WORD mblength;    /* This is the actual number of bytes received */
};

struct lance_tx_desc {
  WORD tmd0;        /* low address of packet */
  BYTE  tmd1_bits;   /* descriptor bits */
  BYTE  tmd1_hadr;   /* high address of packet */
...
struct lance_init_block {
  ...
  BYTE phys_addr[6];     /* Physical ethernet address */
  unsigned filter[2];    /* Multicast filter. */

  /* Receive and transmit ring base, along with extra bits. */
  WORD rx_ptr;    /* receive descriptor addr */
  WORD rx_len;    /* receive len and high addr */
  WORD tx_ptr;    /* transmit descriptor addr */
  WORD tx_len;    /* transmit len and high addr */
...
struct lance_regs {
  WORD rdp;      /* register data port */
  WORD rap;      /* register address port */
};
...
static int lance_open (struct device *dev)
{
  ...
  if (lp->ledma)
    lp->ledma->regs->dma_test = ((DWORD) lp->init_block)
            & 0xff000000;
  ...
  if (!status && lp->auto_select) {
     ...
     */
    volatile struct lance_init_block *ib = lp->init_block;
    volatile DWORD flush;
    BYTE packet[ETH_ZLEN];
