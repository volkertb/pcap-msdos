...
struct mc32_mailbox
{
        WORD     mbox __attribute((packed));
        WORD     data[1] __attribute((packed));
};
...
struct mc32_local 
{
	...
	volatile struct mc32_mailbox *exec_box;
        ...
        WORD tx_halted;
        WORD exec_pending;
        WORD mc_reload_wait;     /* a multicast load request is pending */
	...
	void *rx_ptr[RX_RING_MAX];		/* Data pointers */
        DWORD mc_list_valid;                      /* True when the mclist is set */
};
...
static int __init mc32_probe1(struct net_device *dev, int slot)
{
	...
	struct mc32_local *lp;
        static WORD mca_io_bases[]={
		0x7280,0x7290,
		0x7680,0x7690,
		0x7A80,0x7A90,
		0x7E80,0x7E90
	};
        static DWORD mca_mem_bases[]={
		0x00C0000,
		0x00C4000,
		0x00C8000,
		0x00CC000,
...
static int mc32_command_nowait(struct net_device *dev, WORD cmd, void *data, int len)
{
...
static int mc32_command(struct net_device *dev, WORD cmd, void *data, int len)
{
	struct mc32_local *lp = (struct mc32_local *)dev->priv;
	int ioaddr = dev->base_addr;
	unsigned long flags;
